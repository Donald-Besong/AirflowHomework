...
class Cursor:
    ...
    def execute(
        ...
        try:
            ...
            try:
                _logger.debug(
                    "Cursor's connection._usock state: %s", str(self._c._usock.__dict__)
                )
                _logger.debug(
                    "Cursor's connection._sock is closed: %s", str(self._c._sock.closed)
                )
            except:
                pass
            raise e
    ...
    def __has_valid_columns(
        ...
        q: str = (
            "select 1 from pg_catalog.svv_all_columns where table_name = ? and column_name = ?"
        )
    ...
    def get_schemas(
        ...
        if self.supportSHOWDiscovery() >= self._MIN_SHOW_DISCOVERY_VERSION:
            _logger.debug(
                "Support SHOW command. get_schemas with catalog = %s, schema_pattern = %s",
                catalog,
                schema_pattern,
            )
            ...
            schemas: typing.Tuple = (
                self._metadataAPIPostProcessing.get_schema_post_processing(
                    self._metadataServerAPIHelper.get_schema_server_api(
                        catalog,
                        schema_pattern,
                        ret_empty,
                        self._c.is_single_database_metadata,
                    )
                )
            )
    ...
    def get_catalogs(self: "Cursor") -> typing.Tuple:
        ...
        if self.supportSHOWDiscovery() >= self._MIN_SHOW_DISCOVERY_VERSION:
            ...
            if self._c.is_single_database_metadata is True:
                ...
                self._metadataAPIPostProcessing.set_row_description(
                    self._metadataAPIPostProcessing._get_catalogs_col
                )
            else:
                catalogs: typing.Tuple = (
                    self._metadataAPIPostProcessing.get_catalog_post_processing(
    ...
    def get_tables(
        ...
        if self.supportSHOWDiscovery() >= self._MIN_SHOW_DISCOVERY_VERSION:
            ...
            tables: typing.Tuple = (
                self._metadataAPIPostProcessing.get_table_post_processing(
            ...
            return self.get_tables_legacy_hardcoded_query(
        ...
        if len(types) > 0:
            if schema_pattern_type == "LOCAL_SCHEMA_QUERY":
                ...
                for type in types:
                    ...
                    clauses: typing.Optional[typing.Dict[str, str]] = (
    ...
    def get_columns(
        ...
        if self.supportSHOWDiscovery() >= self._MIN_SHOW_DISCOVERY_VERSION:
            ...
            columns: typing.Tuple = (
                self._metadataAPIPostProcessing.get_column_post_processing(
    ...
    def __schema_pattern_match(
        ...
        if schema_pattern is not None and schema_pattern != "":
            if self._c.is_single_database_metadata is True:
                sql: str = (
                    "select 1 from svv_external_schemas where schemaname like {schema}".format(
                        schema=self.__escape_quotes(schema_pattern)
                    )
                )
                self.execute(sql)
    ...
    def supportSHOWDiscovery(self: "Cursor") -> int:
        for item in self._c.parameter_statuses:
            if item[0] == b"show_discovery":
                try:
                    _logger.debug(
                        "Cluster support SHOW discovery version %d",
                        int(item[1].decode()),
                    )
                    return int(item[1].decode())
                except ValueError:
                    _logger.debug(
                        "SHOW support version unclear: %s, will fall back to version 0",
                        item[1].decode(),
                    )
                    return 0
