...
class MetadataServerAPIHelper(MetadataAPIHelper):
    ...
    def get_schema_server_api(
        ...
        if not ret_empty:
            # Get catalog list
            catalog_list: typing.List = self.call_get_catalog_list(
                catalog, isSingleDatabaseMetaData
            )

            for cur_catalog in catalog_list:
                intermediate_rs.append(
                    self.call_show_schema(cur_catalog, schema_pattern)
                )
                ...
                if self._cursor._SHOW_SCHEMAS_Col_index is None:
                    self._cursor._SHOW_SCHEMAS_Col_index = (
                        self.build_column_name_index_map()
                    )

            _logger.debug(
                "Successfully executed SHOW SCHEMAS for catalog = %s, schemaPattern = %s",
                catalog,
                schema_pattern,
            )
    ...
    def get_table_server_api(
        ...
        if not ret_empty:
            ...
            for cur_catalog in catalog_list:
                # Get schema list
                show_schemas_rs: typing.Tuple = self.call_show_schema(
                    cur_catalog, schema_pattern
                )
    ...
    def get_column_server_api(
        ...
        if not ret_empty:
            ...
            for cur_catalog in catalog_list:
                ...
                for cur_schema in show_schemas_rs:
                    # Get table list
                    cur_schema_name: str = cur_schema[
                        self._cursor._SHOW_SCHEMAS_Col_index[
                            self._SHOW_SCHEMA_schema_name
                        ]
                    ]
    ...
    def call_get_catalog_list(
        self, catalog: str, isSingleDatabaseMetaData: bool
    ) -> typing.List[typing.Tuple]:
        """
    ...
    def call_show_schema_with_like(self, catalog: str, schema: str) -> typing.Tuple:
        ...
        A tuple containing result set for SHOW SCHEMAS: tuple
        """
        sql: str = self._sql_show_schemas_like.format(
            self.quote_ident(catalog), self.quote_literal(schema)
        )
    ...
    def call_show_table_without_like(self, catalog: str, schema: str) -> typing.Tuple:
        ...
        A tuple containing result set for SHOW TABLES: tuple
        """
        sql: str = self._sql_show_tables.format(
            self.quote_ident(catalog), self.quote_ident(schema)
        )
        _logger.debug("Calling Server API: %s", sql)
    ...
    def call_show_column_without_like(
        ...
        sql: str = self._sql_show_columns.format(
            self.quote_ident(catalog), self.quote_ident(schema), self.quote_ident(table)
        )
        _logger.debug("Calling Server API: %s", sql)
