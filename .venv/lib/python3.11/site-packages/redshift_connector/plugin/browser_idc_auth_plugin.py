...
class BrowserIdcAuthPlugin(CommonCredentialsProvider):
    ...
    def add_parameter(
        ...
        _logger.debug(
            "Setting idp_response_timeout = {}".format(self.idp_response_timeout)
        )
        ...
        _logger.debug(
            "Setting idc_client_display_name = {}".format(self.idc_client_display_name)
        )
    ...
    def get_idc_token(self: "BrowserIdcAuthPlugin") -> str:
        ...
        try:
            ...
            auth_code: str = self.fetch_authorization_code(
                code_challenge, register_client_result
            )
            access_token = self.fetch_access_token(
                register_client_result, auth_code, code_verifier
            )
            ...
            _logger.debug(
                "An error occurred while trying to obtain an IdC token : {}".format(
                    str(e)
                )
            )
    ...
    def register_client(self: "BrowserIdcAuthPlugin") -> typing.Dict[str, typing.Any]:
        ...
        register_client_cache_key: str = (
            f"{self.idc_client_display_name}:{self.idc_region}:{self.listen_port}"
        )

        if (
            register_client_cache_key in self.register_client_cache
            and self.register_client_cache[register_client_cache_key][
            ...
            _logger.debug(
                "Valid registerClient result found from cache with expiration time: {}".format(
                    str(
                        self.register_client_cache[register_client_cache_key][
                            "clientSecretExpiresAt"
                        ]
                    )
                )
        ...
        try:
            register_client_result: typing.Dict[str, typing.Any] = (
                self.sso_oidc_client.register_client(
            ...
            raise InterfaceError(
                "IdC authentication failed : Error registering client with IdC."
            )
    ...
    def generate_code_challenge(
        self: "BrowserIdcAuthPlugin", code_verifier: str
    ) -> str:
        """
    ...
    def fetch_authorization_code(
        ...
        try:
            ...
            server_thread = threading.Thread(
                target=self.run_server, args=(listen_socket, state)
            )
            server_thread.start()
            ...
            raise InterfaceError(
                "IdC authentication failed : Timeout while retrieving authorization code."
            )
        except Exception as e:
            raise e
        finally:
    ...
    def fetch_access_token(
        ...
        while time.time() < polling_end_time:
            try:
                ...
                if not response["accessToken"]:
                    raise InterfaceError(
                        "IdC authentication failed : The credential token couldn't be created."
                    )
                _logger.debug(
                    "Length of IdC accessToken: {}".format(len(response["accessToken"]))
                )
        ...
        raise InterfaceError(
            "IdC authentication failed : The request timed out. Authentication wasn't completed."
        )
        ...
        with conn:
            while True:
                ...
                state_idx: int = decoded_part.find(
                    "{}=".format(
                        BrowserIdcAuthPlugin.OAuthParamNames.STATE_PARAMETER_NAME.value
                    )
                )

                if state_idx > -1:
                    received_state: str = decoded_part[
                        state_idx + 6 : decoded_part.find("&", state_idx)
                    ]
                    ...
                    code_idx: int = decoded_part.find(
                        "{}=".format(
                            BrowserIdcAuthPlugin.OAuthParamNames.AUTH_CODE_PARAMETER_NAME.value
                        )
                    )
    ...
    def open_browser(
        ...
        try:
            ...
            _logger.debug(
                "Unable to open the browser. Webbrowser environment is not supported"
            )
    ...
    def get_authorization_token_url(
        ...
        params: typing.Dict[str, str] = {
            ...
            BrowserIdcAuthPlugin.OAuthParamNames.REDIRECT_PARAMETER_NAME.value: str(
                self.redirect_uri
            ),
        ...
        idc_host = (
            self.OIDC_SCHEMA + "." + str(self.idc_region) + "." + self.AMAZON_COM_SCHEMA
        )
    ...
    def get_listen_socket(
        self: "BrowserIdcAuthPlugin", listen_port: int
    ) -> socket.socket:
        """
