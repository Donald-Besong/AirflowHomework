...
class PingCredentialsProvider(SamlCredentialsProvider):
    ...
    def get_saml_assertion(self: "PingCredentialsProvider") -> str:
        ...
        with requests.Session() as session:
            ...
            url: str = (
                "https://{host}:{port}/idp/startSSO.ping?PartnerSpId={sp_id}".format(
            ...
            try:
                _logger.debug(
                    "Issuing GET request for Ping IdP login page using uri=%s verify=%s",
                    url,
                ...
                exec_msg: str = ""
                if "response" in vars():
                    exec_msg = "Get_saml_assertion https response received. Please verify connection properties are correct."
            ...
            try:
                ...
                _logger.debug(
                    "An error occurred while parsing Ping IdP login page: %s", str(e)
                )
            ...
            try:
                _logger.debug(
                    "Issuing authentication request to Ping IdP using uri %s verify %s",
                    url,
                    self.do_verify_ssl_cert(),
                )
            ...
            try:
                ...
                exec_msg = (
                    "An error occurred while parsing Ping IdP authentication response"
                )
                _logger.debug(exec_msg)
                raise InterfaceError(exec_msg) from e
