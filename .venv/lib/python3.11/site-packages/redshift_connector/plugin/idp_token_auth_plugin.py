    ...
    def check_required_parameters(self: "IdpTokenAuthPlugin") -> None:
        ...
        if not self.token:
            ...
            raise InterfaceError(
                "IdC authentication failed: The token must be included in the connection parameters."
            )
        if not self.token_type:
            _logger.error(
                "IdC authentication failed: token_type needs to be provided in connection params"
            )
            raise InterfaceError(
