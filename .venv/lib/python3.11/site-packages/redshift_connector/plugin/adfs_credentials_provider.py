...
class AdfsCredentialsProvider(SamlCredentialsProvider):
    ...
    def add_parameter(
        self: "AdfsCredentialsProvider", info: "RedshiftProperty"
    ) -> None:
    ...
    def form_based_authentication(self: "AdfsCredentialsProvider") -> str:
        ...
        url: str = (
            "https://{host}:{port}/adfs/ls/IdpInitiatedSignOn.aspx?loginToRp={loginToRp}".format(
        ...
        try:
            _logger.debug(
                "Issuing GET request uri=%s verify=%s", url, self.do_verify_ssl_cert()
            )
            ...
            exec_msg: str = ""
            if "response" in vars():
                exec_msg = "ADFS form based authentication https response received but HTTP response code indicates error"
            else:
        ...
        _logger.debug(
            "ADFS form based authentication response length: %s", len(response.text)
        )

        try:
            _logger.debug(
                "Attempt to parse ADFS authentication form from response payload"
            )
            soup = bs4.BeautifulSoup(response.text, features="lxml")
            _logger.debug(
                "Successfully parsed ADFS authentication form from response payload"
            )
        ...
        for inputtag in soup.find_all(re.compile("(INPUT|input)")):
            ...
            if "username" in name.lower():
                _logger.debug(
                    "adding user_name %s under payload key %s", self.user_name, name
                )
        ...
        if action and action.startswith("/"):
            url = "https://{host}:{port}{action}".format(
                host=self.idp_host, port=str(self.idpPort), action=action
            )
        ...
        try:
            _logger.debug(
                "Issuing POST request uri=%s verify=%s", url, self.do_verify_ssl_cert()
            )
            response = requests.post(
                url, data=payload, verify=self.do_verify_ssl_cert()
            )
            _logger.debug("Response code: %s", response.status_code)
            response.raise_for_status()
        except requests.exceptions.HTTPError as e:
        ...
        try:
            ...
            exec_msg = (
                "An unknown error occurred while parsing ADFS authentication response"
            )
            _logger.debug(exec_msg)
            raise InterfaceError(exec_msg) from e
        assertion: str = ""
        ...
        for inputtag in soup.find_all("input"):
            if inputtag.get("name") == "SAMLResponse":
                _logger.debug(
                    "SAMLResponse HTML input tag found in ADFS authentication response payload"
                )
                assertion = inputtag.get("value")

        if assertion == "":
            exec_msg = (
                "Failed to find ADFS access_token in authentication response payload"
            )
            _logger.debug(exec_msg)
