    ...
    def get_credentials(self: "CommonCredentialsProvider") -> NativeTokenHolder:
        ...
        if not credentials or credentials.is_expired():
            if self.disable_cache:
                ...
                _logger.debug(
                    "Auth token Cache enabled - No auth token found from cache : fetching new token"
                )
    ...
    def refresh(self: "CommonCredentialsProvider") -> None:
        ...
        if auth_token is None:
            raise InterfaceError(
                "IdC authentication failed : An error occurred during the request."
            )

        credentials: NativeTokenHolder = NativeTokenHolder(
            access_token=auth_token, expiration=None
        )
