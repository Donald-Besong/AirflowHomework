...
class AzureCredentialsProvider(SamlCredentialsProvider):
    ...
    def azure_oauth_based_authentication(self: "AzureCredentialsProvider") -> str:
        ...
        try:
            ...
            exec_msg = (
                "Too many redirects occurred when requesting Azure OAuth authentication"
            )
            _logger.debug(exec_msg)
            raise InterfaceError(exec_msg) from e
        except requests.exceptions.RequestException as e:
            exec_msg = (
                "A unknown error occurred when requesting Azure OAuth authentication"
            )
            _logger.debug(exec_msg)
            raise InterfaceError(exec_msg) from e

        _logger.debug(
            "Azure Oauth authentication response length: %s", len(response.text)
        )
        ...
        try:
            _logger.debug(
                "attempting to parse Azure Oauth authentication response and grab access_token"
            )
        ...
        if saml_assertion == "":
            ...
            raise InterfaceError(
                "Azure Oauth authentication response access_token is empty"
            )
        ...
        try:
            ...
            exec_msg = "Failed to base64 decode SAML assertion returned from Azure Oauth authentication response payload"
            _logger.debug(exec_msg)
            raise InterfaceError(exec_msg) from e
