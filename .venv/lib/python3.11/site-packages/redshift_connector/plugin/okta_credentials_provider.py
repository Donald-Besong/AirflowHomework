...
class OktaCredentialsProvider(SamlCredentialsProvider):
    ...
    def okta_authentication(self: "OktaCredentialsProvider") -> str:
        ...
        _logger.debug(
            "Okta authentication payload contains username=%s", self.user_name
        )

        try:
            _logger.debug(
                "Issuing Okta authentication request using uri %s verify %s",
                url,
            ...
            if "response" in vars():
                ...
                _logger.debug(
                    "Okta authentication response raised an exception. No response returned."
                )
            _logger.debug(
        ...
        if response is None:
            exec_msg = "Request for authentication returned empty payload. Please verify connection properties are correct."
            _logger.debug(exec_msg)
            raise InterfaceError(exec_msg)
        _logger.debug(
            "Okta_authentication https response length: %s", len(response.content)
        )
    ...
    def handle_saml_assertion(
        ...
        url: str = (
            "https://{host}/home/{app_name}/{app_id}?onetimetoken={session_token}".format(
                host=self.idp_host,
        ...
        try:
            _logger.debug(
                "Issuing request for SAML assertion to Okta IdP using uri=%s verify=%s",
                url,
                self.do_verify_ssl_cert(),
            )
        ...
        _logger.debug(
            "Length of response from Okta with SAML response %s", len(response.content)
        )
