...
class SamlCredentialsProvider(IdpCredentialsProvider):
    ...
    def refresh(self: "SamlCredentialsProvider") -> None:
        ...
        _logger.debug(
            "searching SAML assertion for values matching patterns for RoleArn and PrincipalArn"
        )
        for attr in attrs:
            ...
            if name == "https://aws.amazon.com/SAML/Attributes/Role":
                _logger.debug(
                    "Attribute with name %s found. Checking if pattern match occurs",
                    name,
                )
                for value in values:
        ...
        try:
            ...
            _logger.debug(
                "Extracting temporary AWS credentials from assume_role_with_saml response"
            )
    ...
    def get_cache_key(self: "SamlCredentialsProvider") -> str:
        return (
            "{username}{password}{idp_host}{idp_port}{duration}{preferred_role}".format(
    ...
    def read_metadata(
        self: "SamlCredentialsProvider", doc: bytes
        ...
        try:
            ...
            for idx, namespace in enumerate(SAML_RESP_NAMESPACES):
                ...
                if len(attrs) > 0:
                    _logger.debug(
                        "Attributes found under SAML response namespace %s", namespace
                    )
            ...
            for attr in attrs:
                ...
                attribute_name: str = "{}AttributeValue".format(
                    SAML_RESP_NAMESPACES[namespace_used_idx]
                )
                values: typing.Any = attr.findAll(attribute_name)
                if len(values) == 0 or not values[0].contents:
                    _logger.debug(
                        "No SAML attribute %s found. Continuing to search",
                        attribute_name,
                    )
                ...
                if (
                    ...
                    metadata.set_db_groups(
                        [value.contents[0].lower() for value in values]
                    )
                elif (
                    name == "https://redshift.amazon.com/SAML/Attributes/ForceLowercase"
                ):
