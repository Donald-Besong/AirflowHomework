...
class BrowserAzureCredentialsProvider(SamlCredentialsProvider):
    ...
    def add_parameter(
        self: "BrowserAzureCredentialsProvider", info: RedshiftProperty
    ) -> None:
    ...
    def get_saml_assertion(self: "BrowserAzureCredentialsProvider") -> str:
        ...
        if self.idp_tenant == "" or self.idp_tenant is None:
            BrowserAzureCredentialsProvider.handle_missing_required_property(
                "idp_tenant"
            )
    ...
    def fetch_authorization_token(
        self: "BrowserAzureCredentialsProvider", listen_socket: socket.socket
    ) -> str:
        ...
        try:
            ...
            exec_msg: str = (
                "A socket error occurred when attempting to fetch Azure authorization token"
            )
    ...
    def fetch_saml_response(self: "BrowserAzureCredentialsProvider", token):
        ...
        try:
            ...
            exec_msg = (
                "Too many redirects occurred when requesting Azure authentication"
            )
            _logger.debug(exec_msg)
            raise InterfaceError(exec_msg) from e
    ...
    def wrap_and_encode_assertion(
        self: "BrowserAzureCredentialsProvider", saml_assertion: str
    ) -> str:
        saml_response: str = (
    ...
    def run_server(
        self: "BrowserAzureCredentialsProvider",
        ...
        with conn:
            while True:
                ...
                if state_idx > -1:
                    ...
                    received_code: str = decoded_part[
                        code_idx + 5 : decoded_part.find("&", code_idx)
                    ]
