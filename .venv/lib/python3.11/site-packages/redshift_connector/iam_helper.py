...
class IamHelper(IdpAuthHelper):
    ...
    def get_cluster_credentials_api_type(
        ...
        """
        _logger.debug(
            "Determining which Redshift API to use for retrieving temporary Redshift instance credentials"
        )
        ...
        if not info._is_serverless:
            ...
            if not info.group_federation:
                ...
                raise InterfaceError(
                    FAILED_TO_USE_V2_API_ERROR_MSG.format(
                        "GetClusterCredentials V2 API"
                    )
                )
        elif not info.group_federation:
            _logger.debug(
                "Serverless cluster GetClusterCredentialsAPIType.SERVERLESS_V1"
            )
            ...
            raise InterfaceError(
                FAILED_TO_USE_V2_API_ERROR_MSG.format("GetClusterCredentials V2 API")
            )
    ...
    def set_iam_properties(info: RedshiftProperty) -> RedshiftProperty:
        ...
        provider_type: IamHelper.IAMAuthenticationType = (
            IamHelper.IAMAuthenticationType.NONE
        )
        info.set_is_cname()
        ...
        if info.iam is True:
            if info.region is None:
                _logger.debug(
                    "Setting region via DNS lookup as region was not provided in connection parameters"
                )
                info.set_region_from_endpoint_lookup()

            if (
                info.cluster_identifier is None
                and not info._is_serverless
                and not info.is_cname
            ):
                raise InterfaceError(
            ...
            if metadata is not None:
                ...
                if allow_db_user_override is True:
                    ...
                    if saml_db_user is not None:
                        _logger.debug(
                            "setting db_user to saml_db_user %s", saml_db_user
                        )
                        ...
                        _logger.debug(
                            "setting db_user to profile_db_user %s", profile_db_user
                        )
    ...
    def get_credentials_cache_key(
        ...
        cred_provider: typing.Union[IPlugin, AWSCredentialsProvider],
    ):
        db_groups: str = ""
            ...
            filter(
                ...
                (
                    ...
                    typing.cast(
                        str,
                        (
                            info.serverless_acct_id
                            if info._is_serverless
                            else info.cluster_identifier
                        ),
                    ),
                    typing.cast(
                        str,
                        (
                            info.serverless_work_group
                            if info._is_serverless and info.serverless_work_group
                            else ""
                        ),
                    ),
    ...
    def get_authentication_type(
        ...
        _logger.debug(
            "Inferred authentication type %s from connection parameters", provider_type
        )

        return provider_type

    @staticmethod
    def get_boto3_redshift_client(
        cred_provider: typing.Union[IPlugin, AWSCredentialsProvider],
        info: RedshiftProperty,
    ):
        """
        ...
        try:
            ...
            session_credentials: typing.Dict[str, str] = (
                credentials_holder.get_session_credentials()
            )

            _logger.debug(
                "boto3.client(service_name=%s) being used for IAM auth",
                session_args["service_name"],
            )
    ...
    def set_cluster_identifier(
        ...
        try:
            _logger.debug(
                "Redshift custom domain name in use. Determining cluster identifier."
            )
            response = client.describe_custom_domain_associations(
                CustomDomainName=info.host
            )
            cluster_identifier: str = response["Associations"][0][
            ...
            if info.cluster_identifier is None or info.cluster_identifier == "":
                _logger.debug(
                    "Other Exception when requesting cluster identifier for Redshift with custom domain: %s",
                    e,
                )
                raise e
            else:
    ...
    def set_cluster_host_and_port(
        ...
        try:
            ...
            if (
                ...
                _logger.debug(
                    "retrieving Redshift instance host and port from boto3 redshift client"
                )
                response: dict
                client = IamHelper.get_boto3_redshift_client(cred_provider, info)
                ...
                if info._is_serverless:
                    ...
                    response = client.get_workgroup(
                        workgroupName=info.serverless_work_group
                    )
            ...
            _logger.debug(
                "ClientError when requesting cluster identifier for Redshift with custom domain: %s",
                e,
            )
            raise e
        except Exception as e:
    ...
    def set_cluster_credentials(
        ...
        if cred is None or typing.cast(
            datetime.datetime, cred["Expiration"]
            ...
            provider_type: IamHelper.IAMAuthenticationType = (
                IamHelper.get_authentication_type(cred_provider)
            )
            ...
            _logger.debug(
                "boto3 get_credentials api version: %s will be used",
                get_creds_api_version.value,
            )

            if (
                get_creds_api_version
                ...
                _logger.debug(
                    "Calling get_credentials with parameters %s", get_cred_args
                )
                cred = typing.cast(
                    typing.Dict[str, typing.Union[str, datetime.datetime]],
                ...
                _logger.debug(
                    "Calling get_cluster_credentials_with_iam with parameters %s",
                    request_params,
                )
                ...
                try:
                    ...
                    if info.is_cname:
                        ...
                        _logger.debug(
                            "Retrying calling get_cluster_credentials_with_iam with parameters %s",
                            request_params,
                        )
                ...
                if info.db_user is None or info.db_user == "":
                    raise InterfaceError(
                        "Connection parameter db_user must be specified when using IAM authentication"
                    )
                # https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/redshift/client/get_cluster_credentials.html
                request_params = {
                    "DbUser": info.db_user,
                ...
                _logger.debug(
                    "Calling get_cluster_credentials with parameters %s", request_params
                )
                ...
                try:
                    ...
                    if info.is_cname:
                        ...
                        _logger.debug(
                            "Retrying calling get_cluster_credentials with parameters %s",
                            request_params,
                        )
        ...
        _logger.debug(
            "Using temporary aws credentials with expiration: %s",
            cred.get("Expiration"),
        )
