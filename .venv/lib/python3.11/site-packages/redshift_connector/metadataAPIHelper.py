...
class MetadataAPIHelper:
    ...
    def get_second_fraction(self, data_type: str = None) -> (str, bool):
        ...
        precisions: int = 6
        if re.match(
            r"(time|timetz|timestamp|timestamptz).*\(\d+\)", data_type
    ...
    def get_column_size(
        self, rs_type: str, numeric_precision: int, character_maximum_length: int
    ) -> int:
    ...
    @staticmethod
    def get_decimal_digits(
        rs_type: str, numeric_scale: int, precision: int, customizePrecision: bool
    ) -> int:
    ...
    def get_auto_increment(col_def: str) -> str:
        if col_def is not None and (
            '"identity"' in col_def or "default_identity" in col_def
        ):
            return "YES"
        else:
            return "NO"
