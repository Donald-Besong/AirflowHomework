...
SERVERLESS_HOST_PATTERN: str = (
    r"(.+)\.(.+).redshift-serverless(-dev)?\.amazonaws\.com(.)*"
)
SERVERLESS_WITH_WORKGROUP_HOST_PATTERN: str = (
    r"(.+)\.(.+)\.(.+).redshift-serverless(-dev)?\.amazonaws\.com(.)*"
)
...
class RedshiftProperty:
    ...
    def is_serverless_host(self: "RedshiftProperty") -> bool:
        ...
        return bool(
            re.fullmatch(pattern=SERVERLESS_HOST_PATTERN, string=str(self.host))
        ) or bool(
            re.fullmatch(
                pattern=SERVERLESS_WITH_WORKGROUP_HOST_PATTERN, string=str(self.host)
            )
        )
    ...
    def is_provisioned_host(self: "RedshiftProperty") -> bool:
        ...
        return bool(
            re.fullmatch(pattern=PROVISIONED_HOST_PATTERN, string=str(self.host))
        )
    ...
    def set_is_cname(self: "RedshiftProperty") -> None:
        ...
        is_cname: bool = False
        _logger.debug(
            "determining if host indicates Redshift instance with custom name"
        )
        ...
        if self.is_provisioned_host:
            ...
            if self.serverless_work_group is not None:
                ...
                _logger.debug(
                    "cluster identified as Redshift serverless with with custom name"
                )
                is_cname = True
        else:
            _logger.debug(
                "cluster identified as Redshift provisioned with with custom name/NLB"
            )
            is_cname = True
    ...
    def set_serverless_acct_id(self: "RedshiftProperty") -> None:
        ...
        for serverless_pattern in (
            SERVERLESS_WITH_WORKGROUP_HOST_PATTERN,
            SERVERLESS_HOST_PATTERN,
        ):
            ...
            if m2:
                ...
                self.put(
                    key="serverless_acct_id",
                    value=m2.group(typing.cast(int, m2.lastindex) - 1),
                )
    ...
    def set_region_from_endpoint_lookup(self: "RedshiftProperty") -> None:
        ...
        if not all((self.host, self.port)):
            _logger.debug(
                "host and port were unspecified, exiting set_region_from_endpoint_lookup"
            )
            return
        try:
            addr_response: typing.List[
                ...
                host=self.host, port=self.port, family=socket.AF_INET
            )
            _logger.debug("%s", addr_response)
            host_response: typing.Tuple[str, typing.List, typing.List] = (
                socket.gethostbyaddr(addr_response[0][4][0])
            )
            ...
            msg: str = (
                "Unable to automatically determine AWS region from host {} port {}. Please check host and port connection parameters are correct.".format(
                    self.host, self.port
                )
            )
            _logger.debug(msg)
    ...
    def set_serverless_work_group_from_host(self: "RedshiftProperty") -> None:
        ...
        m2 = re.fullmatch(
            pattern=SERVERLESS_WITH_WORKGROUP_HOST_PATTERN, string=self.host
        )
