...
class IdpAuthHelper:
    ...
    def get_pkg_version(module_name: str) -> Version:
        ...
        try:
            ...
            ModuleNotFoundError
        ):  # if importlib is not present, fallback to pkg_resources
    ...
    def set_auth_properties(info: RedshiftProperty):
        ...
        if info.iam is True:
            ...
            _logger.debug(
                "botocore version: %s", IdpAuthHelper.get_pkg_version("botocore")
            )
    ...
    def read_auth_profile(
        ...
        _logger.debug(
            "Received %s authentication profiles",
            len(response["AuthenticationProfiles"]),
        )
        # the first matching authentication profile will be used
        profile_content: typing.Union[str] = response["AuthenticationProfiles"][0][
        ...
        try:
            ...
            raise ProgrammingError(
                "Unable to decode the JSON content of the Redshift authentication profile: {}".format(
                    auth_profile
                )
            )
    ...
    def load_credentials_provider(info: RedshiftProperty) -> IPlugin:
        ...
        try:
            ...
            try:
                predefined_idp: str = "redshift_connector.plugin.{}".format(
                    info.credentials_provider
                )
                ...
                raise InterfaceError(
                    "Invalid IdP specified in credential_provider connection parameter: "
                    + info.credentials_provider
                )

        if not issubclass(klass, IPlugin):
            raise InterfaceError(
                "Invalid value passed to credentials_provider: {}".format(
                    info.credentials_provider
                )
            )
        else:
