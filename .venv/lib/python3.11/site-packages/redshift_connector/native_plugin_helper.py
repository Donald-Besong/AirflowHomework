...
class NativeAuthPluginHelper:
    ...
    def set_native_auth_plugin_properties(info: "RedshiftProperty") -> None:
        ...
        if info.credentials_provider:
            ...
            idp_token: str = NativeAuthPluginHelper.get_native_auth_plugin_credentials(
                info
            )
            if idp_token:
    ...
    def get_native_auth_plugin_credentials(info: "RedshiftProperty") -> str:
        ...
        if info.credentials_provider:
            ...
            if not isinstance(provider, INativePlugin):
                _logger.debug(
                    "Native auth will not be used, no credentials provider specified"
                )
                return ""
        else:
            raise InterfaceError(
                "Required credentials_provider parameter is null or empty: {}".format(
                    info.credentials_provider
                )
            )

        _logger.debug(
            "Native IDP Credential Provider %s:%s", provider, info.credentials_provider
        )
        _logger.debug("Calling provider.getCredentials()")

        # Provider will cache the credentials, it's OK to call get_credentials() here
        credentials: "NativeTokenHolder" = typing.cast(
            "NativeTokenHolder", provider.get_credentials()
        )
        ...
        if credentials is None or (
            credentials.expiration is not None and credentials.is_expired()
        ):
            # get idp token
            plugin: INativePlugin = provider
            _logger.debug(
                "Unable to get IdP token from cache. Calling plugin.get_idp_token()"
            )
