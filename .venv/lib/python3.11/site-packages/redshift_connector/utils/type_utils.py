...
def vector_in(data: bytes, idx: int, length: int) -> typing.List:
    return eval(
        "[" + data[idx : idx + length].decode(_client_encoding).replace(" ", ",") + "]"
    )
...
def timestamp_recv_integer(
    data: bytes, offset: int, length: int
) -> typing.Union[Datetime, str, float]:
...
# to local.
def timestamptz_recv_integer(
    data: bytes, offset: int, length: int
) -> typing.Union[str, Datetime, int]:
...
def numeric_in_binary(data: bytes, offset: int, length: int, scale: int) -> Decimal:
    ...
    if length == 8 or length == 16:
        raw_value = int.from_bytes(
            data[offset : offset + length], byteorder="big", signed=True
        )
    else:
...
def interval_recv_integer(
    data: bytes, offset: int, length: int
) -> typing.Union[Timedelta, Interval]:
    microseconds, days, months = typing.cast(
        typing.Tuple[int, ...], qhh_unpack(data, offset)
    )
...
def time_recv_binary(data: bytes, offset: int, length: int) -> time:
    ...
    micros: float = (
        r * 1000
    )  # maximum of six digits of precision for fractional seconds.
    q, r = divmod(q, 60)
    seconds: float = r
    q, r = divmod(q, 60)
    minutes: float = r
    ...
    if length != 8:
        ...
        if data[idx_tz : idx_tz + 1] == b".":
            ...
            for idx in range(idx_tz + 1, len(data)):
                if (
                    data[idx] == 43 or data[idx] == 45
                ):  # +/- char indicates start of tz offset
...
def _parse_array(
    adapter: typing.Optional[typing.Callable], data: bytes, offset: int, length: int
) -> typing.List:
...
redshift_types: typing.DefaultDict[int, typing.Tuple[int, typing.Callable]] = (
    defaultdict(
...
def text_out(
    v: typing.Union[PGText, PGVarchar, PGJson, PGJsonb, PGTsvector, str]
) -> bytes:
...
py_types: typing.Dict[
    typing.Union[type, int], typing.Tuple[int, int, typing.Callable]
] = {
