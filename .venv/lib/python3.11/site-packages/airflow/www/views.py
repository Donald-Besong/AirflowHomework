        ...
        if not isinstance(item, TaskGroup):
                ...
                instances = [
                    {
                        ...
                        "try_number": wwwutils.get_try_count(
                            task_instance._try_number, task_instance.state
                        ),
            ...
            return {
                ...
                "has_outlet_datasets": any(
                    isinstance(i, Dataset) for i in (item.outlets or [])
                ),
...
class Airflow(AirflowBaseView):
    ...
    def index(self):
        ...
        with create_session() as session:
            ...
            if get_auth_manager().is_authorized_view(
                ...
                import_errors = select(errors.ImportError).order_by(
                    errors.ImportError.id
                )
    ...
    def grid_data(self):
        ...
        encoded_runs = [
            wwwutils.encode_dag_run(dr, json_encoder=utils_json.WebEncoder)
            for dr in dag_runs
        ]
        data = {
...
class AutocompleteView(AirflowBaseView):
    ...
    def autocomplete(self, session: Session = NEW_SESSION):
        ...
        dag_ids_query = select(
            ...
            ~DagModel.is_subdag, DagModel.is_active, DagModel.dag_id.ilike(f"%{query}%")
        )
