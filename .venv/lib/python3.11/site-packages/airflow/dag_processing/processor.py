...
class DagFileProcessor(LoggingMixin):
    ...
    def update_import_errors(
        ...
        existing_import_error_files = [
            x.filename for x in session.query(errors.ImportError.filename).all()
        ]
        ...
        for dag in dagbag.dags.values():
            ...
            if message:
                self.dag_warnings.add(
                    DagWarning(dag.dag_id, DagWarningType.NONEXISTENT_POOL, message)
                )
            for subdag in dag.subdags:
                ...
                if message:
                    self.dag_warnings.add(
                        DagWarning(
                            subdag.dag_id, DagWarningType.NONEXISTENT_POOL, message
                        )
                    )
    ...
    def _execute_dag_callbacks(
        self, dagbag: DagBag, request: DagCallbackRequest, session: Session
    ):
    ...
    def _execute_task_callbacks(
        ...
        ti.handle_failure(
            error=request.msg, test_mode=self.UNIT_TEST_MODE, session=session
        )
