    ...
    with create_session() as session:
        ...
        if (
            ...
            task_instance.xcom_push(
                key=XCOM_RETURN_KEY, value=xcom_value, session=session
            )
...
def _set_try_number(
    *, task_instance: TaskInstance | TaskInstancePydantic, value: int
) -> None:
    """
...
class TaskInstance(Base, LoggingMixin):
    ...
    def get_task_instance(
        ...
        query = (
            session.query(*TaskInstance.__table__.columns)
            if select_columns
    ...
    def next_retry_datetime(self):
        ...
        if self.task.retry_exponential_backoff:
            ...
            min_backoff = math.ceil(
                delay.total_seconds() * (2 ** (self.try_number - 2))
            )
    ...
    def _run_raw_task(
        ...
        Stats.incr(
            f"ti.start.{self.task.dag_id}.{self.task.task_id}", tags=self.stats_tags
        )
        ...
        with set_current_task_instance_session(session=session):
            ...
            try:
                if not mark_success:
                    self._execute_task_with_callbacks(
                        context, test_mode, session=session
                    )
                if not test_mode:
                ...
                if not test_mode:
                    self.refresh_from_db(lock_for_update=True, session=session)
                _run_finished_callback(
                    callbacks=self.task.on_skipped_callback, context=context
                )
                session.commit()
                ...
                # Same metric with tagging
                Stats.incr(
                    "ti.finish", tags={**self.stats_tags, "state": str(self.state)}
                )
            ...
            _run_finished_callback(
                callbacks=self.task.on_success_callback, context=context
            )
