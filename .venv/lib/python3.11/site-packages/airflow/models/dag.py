...
ScheduleArg = Union[
    ArgNotSet, ScheduleInterval, Timetable, BaseDatasetEventInput, Collection["Dataset"]
]
...
class DAG(LoggingMixin):
    ...
    def _upgrade_outdated_dag_access_control(access_control=None):
        ...
        for role, perms in access_control.items():
            updated_access_control[role] = {
                new_perm_mapping.get(perm, perm) for perm in perms
            }
            ...
            while dr.state == DagRunState.RUNNING:
                ...
                for ti in scheduled_tis:
                    try:
                        ...
                        _run_task(
                            ti=ti, inline_trigger=not triggerer_running, session=session
                        )
                    except Exception:
    ...
    def simplify_dataset_expression(self, dataset_expression) -> dict | None:
        ...
        return {
            new_key: [
                self.simplify_dataset_expression(item)
                for item in dataset_expression["__var"]
            ]
        }
    ...
    def bulk_write_to_db(
        ...
        for dag in dags:
            ...
            for task in dag.tasks:
                dataset_outlets = [
                    x for x in task.outlets or [] if isinstance(x, Dataset)
                ]
        ...
        for dag_id, uri_list in dag_references.items():
            dag_refs_needed = {
                DagScheduleDatasetReference(
                    dataset_id=stored_datasets[uri].id, dag_id=dag_id
                )
                for uri in uri_list
            }
            dag_refs_stored = set(
        ...
        for (dag_id, task_id), uri_list in outlet_references.items():
            task_refs_needed = {
                TaskOutletDatasetReference(
                    dataset_id=stored_datasets[uri].id, dag_id=dag_id, task_id=task_id
                )
                for uri in uri_list
            }
    ...
    def get_serialized_fields(cls):
        ...
        if not cls.__serialized_fields:
            ...
            cls.__serialized_fields = (
                frozenset(vars(DAG(dag_id="test"))) - exclusion_list
            )
