...
class EmrStartNotebookExecutionOperator(BaseOperator):
    ...
    def execute(self, context: Context):
        execution_engine = {
            "Id": self.cluster_id,
            "Type": "EMR",
            "MasterInstanceSecurityGroupId": self.master_instance_security_group_id
            or "",
        }
        emr_hook = EmrHook(aws_conn_id=self.aws_conn_id)
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if wait_for_completion is not None:
            ...
            self.wait_policy = (
                WaitPolicy.WAIT_FOR_COMPLETION if wait_for_completion else None
            )
...
class EmrServerlessCreateApplicationOperator(BaseOperator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        waiter_max_attempts = (
            25 if waiter_max_attempts is NOTSET else waiter_max_attempts
        )
    ...
    def execute_complete(
        ...
        if validated_event["status"] != "success":
            raise AirflowException(
                f"Trigger error: Application failed to start, event is {validated_event}"
            )
...
class EmrServerlessStartJobOperator(BaseOperator):
    ...
    def persist_links(self, context: Context):
        ...
        if self.is_monitoring_in_job_override(
            ...
            log_group_name = cloudwatch_config.get(
...
class EmrServerlessStopApplicationOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> None:
        ...
        if self.force_stop:
            ...
            if count > 0:
                ...
                self.hook.get_waiter("no_job_running").wait(
                    applicationId=self.application_id,
                    states=list(
                        self.hook.JOB_INTERMEDIATE_STATES.union({"CANCELLING"})
                    ),
                    WaiterConfig={
                        "Delay": self.waiter_delay,
                        "MaxAttempts": self.waiter_max_attempts,
        ...
        if self.wait_for_completion:
            ...
            self.log.info(
                "EMR serverless application %s stopped successfully",
                self.application_id,
            )
...
class EmrServerlessDeleteApplicationOperator(EmrServerlessStopApplicationOperator):
    ...
    def execute_complete(
        ...
        if validated_event["status"] == "success":
            self.log.info(
                "EMR serverless application %s deleted successfully",
                self.application_id,
            )
