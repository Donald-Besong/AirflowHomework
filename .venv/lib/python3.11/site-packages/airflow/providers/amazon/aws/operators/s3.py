...
class S3CreateObjectOperator(AwsBaseOperator[S3Hook]):
    ...
    def get_openlineage_facets_on_start(self):
        ...
        bucket, key = self.hook.get_s3_bucket_key(
            self.s3_bucket, self.s3_key, "dest_bucket", "dest_key"
        )
...
class S3DeleteObjectsOperator(AwsBaseOperator[S3Hook]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not exactly_one(
            keys is None,
            all(var is None for var in [prefix, from_datetime, to_datetime]),
        ):
    ...
    def execute(self, context: Context):
        if not exactly_one(
            ...
            all(
                var is None
                for var in [self.prefix, self.from_datetime, self.to_datetime]
            ),
        ):
            raise AirflowException(
                "Either keys or at least one of prefix, from_datetime, to_datetime should be set."
        ...
        if isinstance(self.from_datetime, str):
            self.from_datetime = parser.parse(self.from_datetime).replace(
                tzinfo=pytz.UTC
            )
...
class S3FileTransformOperator(AwsBaseOperator[S3Hook]):
    ...
    template_fields: Sequence[str] = aws_template_fields(
        "source_s3_key", "dest_s3_key", "script_args"
    )
    ...
    def execute(self, context: Context):
        ...
        source_s3 = S3Hook(
            aws_conn_id=self.source_aws_conn_id, verify=self.source_verify
        )
...
class S3ListPrefixesOperator(AwsBaseOperator[S3Hook]):
    ...
    **Example**:
    ...
    def execute(self, context: Context):
        ...
        return self.hook.list_prefixes(
