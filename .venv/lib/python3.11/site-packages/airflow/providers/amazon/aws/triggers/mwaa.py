...
class MwaaDagRunCompletedTrigger(AwsBaseWaiterTrigger):
    ...
    def __init__(
        ...
        *,
        ...
        self.success_states = (
            set(success_states) if success_states else {DagRunState.SUCCESS.value}
        )
        self.failure_states = (
            set(failure_states) if failure_states else {DagRunState.FAILED.value}
        )

        if len(self.success_states & self.failure_states):
            raise ValueError(
                "success_states and failure_states must not have any values in common"
            )

        in_progress_states = (
            {s.value for s in DagRunState} - self.success_states - self.failure_states
        )
