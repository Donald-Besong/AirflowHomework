...
class BaseSessionFactory(LoggingMixin):
    ...
    def _apply_session_kwargs(self, session):
        ...
        if self.conn.session_kwargs.get("region_name", None) is not None:
            session.set_config_variable(
                "region", self.conn.session_kwargs["region_name"]
            )
...
class AwsGenericHook(BaseHook, Generic[BaseAwsConnection]):
    ...
    def get_waiter(
        ...
        if (
            ...
            and config_overrides is not None
            and "acceptors" in config_overrides
        ):
            raise ValueError(
                'parameters must be None when "acceptors" is included in config_overrides'
            )
