...
class SageMakerHook(AwsBaseHook):
    ...
    def describe_training_job_with_log(
        ...
        if stream_names:
            for idx, event in self.multi_stream_iter(
    ...
    def list_training_jobs(
        ...
        list_training_jobs_request = partial(
            self.get_conn().list_training_jobs, **config
        )
    ...
    def count_processing_jobs_by_name(
        ...
        """
        try:
            jobs = self.get_conn().list_processing_jobs(
                NameContains=processing_job_name
            )
    ...
    def stop_pipeline(
        ...
        for retries in reversed(range(5)):
            try:
                self.conn.stop_pipeline_execution(
                    PipelineExecutionArn=pipeline_exec_arn
                )
            except ClientError as ce:
    ...
    def _describe_auto_ml_job(self, job_name: str):
        ...
        self.log.info(
            "%s's current step: %s", job_name, res["AutoMLJobSecondaryStatus"]
        )
        return res
            ...
            params_dict.update(
                {
                    "AutoMLJobConfig": {
                        "CompletionCriteria": {
                            "MaxAutoMLJobRuntimeInSeconds": time_limit
                        }
                    }
                }
            )
