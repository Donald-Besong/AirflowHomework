...
class DmsHook(AwsBaseHook):
    ...
    def describe_replication_configs(
        self, filters: list[dict] | None = None, **kwargs
    ) -> list[dict]:
        """
    ...
    def describe_replications(
        self, filters: list[dict[str, Any]] | None = None, **kwargs
    ) -> list[dict]:
        """
    ...
    def delete_replication_config(
        ...
        try:
            ...
            self.conn.delete_replication_config(
    ...
    def start_replication(
        ...
        if cdc_start_time:
            additional_args["CdcStartTime"] = (
                cdc_start_time
                if isinstance(cdc_start_time, datetime)
                else parser.parse(cdc_start_time)
            )
        if cdc_start_pos:
    ...
    def get_provision_status(self, replication_config_arn: str) -> str:
        ...
        result = self.describe_replications(
            filters=[
                {"Name": "replication-config-arn", "Values": [replication_config_arn]}
            ]
        )
