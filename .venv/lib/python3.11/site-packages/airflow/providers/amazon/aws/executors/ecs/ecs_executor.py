...
class AwsEcsExecutor(BaseExecutor):
    ...
    def __describe_tasks(self, task_arns):
        ...
        for i in range(0, len(task_arns), self.DESCRIBE_TASKS_BATCH_SIZE):
            ...
            describe_tasks_response = BotoDescribeTasksSchema().load(
                boto_describe_tasks
            )

            all_task_descriptions["tasks"].extend(describe_tasks_response["tasks"])
            all_task_descriptions["failures"].extend(
                describe_tasks_response["failures"]
            )
    ...
    def execute_async(
        ...
        if executor_config and (
            ...
            raise ValueError(
                'Executor Config should never override "name" or "command"'
            )
        if len(command) == 1:
            from airflow.executors.workloads import ExecuteTask
