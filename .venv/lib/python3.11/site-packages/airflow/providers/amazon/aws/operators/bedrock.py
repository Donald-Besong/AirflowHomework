...
class BedrockCustomizeModelOperator(AwsBaseOperator[BedrockHook]):
    ...
    def execute_complete(
        ...
        return self.hook.conn.get_model_customization_job(
            jobIdentifier=validated_event["job_name"]
        )["jobArn"]
    ...
    def execute(self, context: Context) -> dict:
        ...
        task_description = (
            f"Bedrock model customization job {self.job_name} to complete."
        )
        if self.deferrable:
...
class BedrockCreateProvisionedModelThroughputOperator(AwsBaseOperator[BedrockHook]):
    ...
    def execute_complete(
        ...
        self.log.info(
            ...
            validated_event["provisioned_model_id"],
        )
...
class BedrockIngestDataOperator(AwsBaseOperator[BedrockAgentHook]):
    ...
    def execute_complete(
        ...
        if validated_event["status"] != "success":
            raise AirflowException(
                f"Error while running ingestion job: {validated_event}"
            )

        self.log.info(
            "Bedrock ingestion job `%s` complete.", validated_event["ingestion_job_id"]
        )
...
class BedrockBatchInferenceOperator(AwsBaseOperator[BedrockHook]):
    ...
    def execute_complete(
        ...
        if validated_event["status"] != "success":
            raise AirflowException(
                f"Error while running {self.activity}: {validated_event}"
            )
    ...
    def execute(self, context: Context) -> str:
        ...
        self.log.info(
            "%s '%s' started with ARN: %s", self.activity, self.job_name, job_arn
        )
