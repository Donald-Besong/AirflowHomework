...
class SageMakerProcessingOperator(SageMakerBaseOperator):
    ...
    def execute(self, context: Context) -> dict:
        ...
        if self.deferrable and self.wait_for_completion:
            response = self.hook.describe_processing_job(
                self.config["ProcessingJobName"]
            )
    ...
    def execute_complete(
        ...
        self.serialized_job = serialize(
            self.hook.describe_processing_job(validated_event["job_name"])
        )
        self.log.info("%s completed successfully.", self.task_id)
        return {"Processing": self.serialized_job}
...
class SageMakerEndpointOperator(SageMakerBaseOperator):
    ...
    def execute(self, context: Context) -> dict:
        ...
        try:
            ...
            if self.operation == "create" and ce.response["Error"][
                "Message"
            ].startswith("Cannot create already existing endpoint"):
    ...
    def execute_complete(
        ...
        return {
            ...
            "Endpoint": serialize(
                self.hook.describe_endpoint(response["EndpointName"])
            ),
        }
...
class SageMakerTransformOperator(SageMakerBaseOperator):
    ...
    def execute(self, context: Context) -> dict:
        ...
        if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
            raise AirflowException(
                f"Sagemaker transform Job creation failed: {response}"
            )

        transform_job_url = SageMakerTransformJobLink.format_str.format(
            aws_domain=SageMakerTransformJobLink.get_aws_domain(
        ...
        self.log.info(
            "You can monitor this SageMaker Transform job at %s", transform_job_url
        )
...
class SageMakerTuningOperator(SageMakerBaseOperator):
    ...
    def execute_complete(
        ...
        return {
            "Tuning": serialize(
                self.hook.describe_tuning_job(validated_event["job_name"])
            )
        }
...
class SageMakerTrainingOperator(SageMakerBaseOperator):
    ...
    def execute(self, context: Context) -> dict:
        ...
        if self.deferrable and self.wait_for_completion:
            description = self.hook.describe_training_job(
                self.config["TrainingJobName"]
            )
...
class SageMakerStartPipelineOperator(SageMakerBaseOperator):
    ...
    def execute_complete(
        ...
        if validated_event["status"] != "success":
            raise AirflowException(
                f"Failure during pipeline execution: {validated_event}"
            )
...
class SageMakerStopNotebookOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        if self.wait_for_completion:
            self.log.info(
                "Waiting for SageMaker notebook %s to stop", self.instance_name
            )
