...
class DmsDeleteTaskOperator(AwsBaseOperator[DmsHook]):
    ...
    def execute(self, context: Context):
        ...
        :return: replication task arn
        """
        self.hook.delete_replication_task(
...
class DmsStartTaskOperator(AwsBaseOperator[DmsHook]):
    ...
    def execute(self, context: Context):
        ...
        self.log.info(
            "DMS replication task(%s) is starting.", self.replication_task_arn
        )
...
class DmsStopTaskOperator(AwsBaseOperator[DmsHook]):
    ...
    def execute(self, context: Context):
        ...
        self.log.info(
            "DMS replication task(%s) is stopping.", self.replication_task_arn
        )
...
class DmsCreateReplicationConfigOperator(AwsBaseOperator[DmsHook]):
    ...
    def execute(self, context: Context) -> str:
        ...
        self.log.info(
            "DMS replication config(%s) has been created.", self.replication_config_id
        )
        return resp
...
class DmsDeleteReplicationConfigOperator(AwsBaseOperator[DmsHook]):
    ...
    def execute(self, context: Context) -> None:
            ...
            filters=[
                {
                    ...
                    "Values": [self.replication_config_arn],
                }
            ]
        )
        ...
        if self.deferrable:
            ...
            if (
                provision_status not in self.TERMINAL_PROVISION_STATES
            ):  # not deprovisioned:
    ...
    def handle_delete_wait(self):
        if self.wait_for_completion:
            if self.deferrable:
                ...
                self.log.info(
                    "DMS replication config(%s) deleted.", self.replication_config_arn
                )
    ...
    def retry_execution(self, context, event=None):
        self.replication_config_arn = event.get("replication_config_arn")
        self.log.info(
            "Retrying replication config(%s) deletion.", self.replication_config_arn
        )
        ...
        **kwargs,
        ...
        if self.cdc_start_time and self.cdc_start_pos:
            raise AirflowException(
                "Only one of cdc_start_time or cdc_start_pos should be provided."
            )
    ...
    def execute(self, context: Context):
        ...
        if provision_status == "deprovisioning":
            ...
            if self.deferrable:
                ...
                self.log.info(
                    "Replication deprovisioning complete. Provision status: %s",
                    provision_status,
                )
        ...
        if (
            ...
            if self.wait_for_completion:
                self.log.info(
                    "Waiting for %s replication to complete.",
                    self.replication_config_arn,
                )

                if self.deferrable:
                    self.log.info(
                        "Deferring until %s replication completes.",
                        self.replication_config_arn,
                    )
                    self.defer(
                ...
                self.log.info(
                    "Replication(%s) has completed.", self.replication_config_arn
                )

        else:
            self.log.info(
                "Replication(%s) is not in startable state.",
                self.replication_config_arn,
            )
            self.log.info(
                "Status: %s Provision status: %s", current_status, provision_status
            )
...
class DmsStopReplicationOperator(AwsBaseOperator[DmsHook]):
    ...
    def execute(self, context: Context) -> None:
        ...
        if current_state.lower() in self.STOPPED_STATES:
            self.log.info(
                "DMS replication config(%s) is already stopped.",
                self.replication_config_arn,
            )
        else:
            ...
            self.log.info(
                "Stopping DMS replication config(%s). Current status: %s",
                self.replication_config_arn,
                status,
            )
            ...
            if self.wait_for_completion:
                self.log.info(
                    "Waiting for %s replication to stop.", self.replication_config_arn
                )

                if self.deferrable:
                    self.log.info(
                        "Deferring until %s replication stops.",
                        self.replication_config_arn,
                    )
                    self.defer(
