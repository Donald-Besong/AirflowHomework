...
class GlueJobOperator(BaseOperator):
    ...
    def execute(self, context: Context):
        ...
        if self.deferrable:
            ...
            self.log.info(
                "AWS Glue Job: %s. Run Id: %s", self.job_name, self._job_run_id
            )
...
class GlueDataQualityOperator(AwsBaseOperator[GlueDataQualityHook]):
    ...
    def validate_inputs(self) -> None:
        ...
        if self.data_quality_ruleset_kwargs.get("TargetTable"):
            ...
            if not target_table.get("TableName") or not target_table.get(
                "DatabaseName"
            ):
                raise AttributeError(
                    "Target table must have DatabaseName and TableName"
                )
...
class GlueDataQualityRuleSetEvaluationRunOperator(AwsBaseOperator[GlueDataQualityHook]):
    ...
    def execute_complete(
        ...
        if validated_event["status"] != "success":
            raise AirflowException(
                f"Error: AWS Glue data quality ruleset evaluation run: {validated_event}"
            )
...
class GlueDataQualityRuleRecommendationRunOperator(
    AwsBaseOperator[GlueDataQualityHook]
):
    """
    ...
    def execute(self, context: Context) -> str:
        ...
        self.log.info(
            "Submitting AWS Glue data quality recommendation run with %s",
            self.datasource,
        )
    ...
    def execute_complete(
        ...
        if validated_event["status"] != "success":
            raise AirflowException(
                f"Error: AWS Glue data quality rule recommendation run: {validated_event}"
            )

        if self.show_results:
            self.hook.log_recommendation_results(
                run_id=validated_event["recommendation_run_id"]
            )
