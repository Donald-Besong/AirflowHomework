...
class AwsAuthManager(BaseAuthManager[AwsAuthManagerUser]):
    ...
    def is_authorized_backfill(
        ...
        *,
        ...
        return self.avp_facade.is_authorized(
            method=method,
            entity_type=AvpEntities.BACKFILL,
            user=user,
    ...
    def filter_authorized_menu_items(
        ...
        return [
            menu_item
            for menu_item in menu_items
            if _has_access_to_menu_item(requests[menu_item.value])
        ]
        ...
        facade_requests: Sequence[IsAuthorizedRequest] = [
            {
                ...
                "entity_type": AvpEntities.CONNECTION,
                "entity_id": (
                    cast("ConnectionDetails", request["details"]).conn_id
        ...
        facade_requests: Sequence[IsAuthorizedRequest] = [
            {
                ...
                "entity_type": AvpEntities.DAG,
                "entity_id": (
                    cast("DagDetails", request["details"]).id
                    if request.get("details")
                    else None
                ),
                "context": (
                    {
                        "dag_entity": {
                            "string": cast(
                                "DagAccessEntity", request["access_entity"]
                            ).value,
                        },
                    }
        ...
        facade_requests: Sequence[IsAuthorizedRequest] = [
            {
                ...
                "entity_type": AvpEntities.POOL,
                "entity_id": (
                    cast("PoolDetails", request["details"]).name
                    if request.get("details")
                    else None
                ),
            }
        ...
        facade_requests: Sequence[IsAuthorizedRequest] = [
            {
                ...
                "entity_type": AvpEntities.VARIABLE,
                "entity_id": (
                    cast("VariableDetails", request["details"]).key
                    if request.get("details")
                    else None
                ),
            }
    ...
    def filter_authorized_dag_ids(
        ...
        *,
        ...
        requests: dict[str, dict[ResourceMethod, IsAuthorizedRequest]] = defaultdict(
            dict
        )
        ...
        return {
            dag_id for dag_id in dag_ids if _has_access_to_dag(requests[dag_id][method])
        }
