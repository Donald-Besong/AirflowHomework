...
CAN_NOT_DELETE_MSG = (
    "A cluster can not be deleted with attached {compute}.  Deleting {count} {compute}."
)
...
def _create_compute(
    ...
    if compute == "nodegroup" and nodegroup_name:
        ...
        if wait_for_completion:
            log.info(
                "Waiting for Fargate profile to provision.  This will take some time."
            )
            wait(
...
class EksCreateClusterOperator(AwsBaseOperator[EksHook]):
            ...
            if (self.compute == "nodegroup") and not self.nodegroup_role_arn:
                raise ValueError(
                    MISSING_ARN_MSG.format(
                        compute=NODEGROUP_FULL_NAME, requirement="nodegroup_role_arn"
                    )
                )
    ...
    def deferrable_create_cluster_next(
    ...
    def execute(self, context: Context):
        ...
        if isinstance(self.nodegroup_subnets, str):
            ...
            if self.nodegroup_subnets != "":
                try:
                    nodegroup_subnets_list = cast(
                        "list", literal_eval(self.nodegroup_subnets)
                    )
                except ValueError:
                    self.log.warning(
...
class EksDeleteClusterOperator(AwsBaseOperator[EksHook]):
    ...
    def delete_any_nodegroups(self) -> None:
        ...
        if nodegroups:
            self.log.info(
                CAN_NOT_DELETE_MSG.format(
                    compute=NODEGROUP_FULL_NAME, count=len(nodegroups)
                )
            )
            for group in nodegroups:
                self.hook.delete_nodegroup(
            ...
            self.log.info(
                "Waiting for all nodegroups to delete.  This will take some time."
            )
    ...
    def delete_any_fargate_profiles(self) -> None:
        ...
        """
        fargate_profiles = self.hook.list_fargate_profiles(
            clusterName=self.cluster_name
        )
        if fargate_profiles:
            self.log.info(
                CAN_NOT_DELETE_MSG.format(
                    compute=FARGATE_FULL_NAME, count=len(fargate_profiles)
                )
            )
            self.log.info(
...
class EksDeleteFargateProfileOperator(AwsBaseOperator[EksHook]):
    ...
    def execute(self, context: Context):
        ...
        if self.deferrable:
            ...
            self.log.info(
                "Waiting for Fargate profile to delete.  This will take some time."
            )
            self.hook.conn.get_waiter("fargate_profile_deleted").wait(
                clusterName=self.cluster_name,
