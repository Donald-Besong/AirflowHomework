...
class HttpHook(BaseHook):
    ...
    def get_conn(
        ...
        if connection.extra:
            session = self._configure_session_from_extra(
                session, connection, extra_options
            )
    ...
    def _configure_session_from_extra(
        ...
        session.verify = extra_options.pop(
            "verify", extra_options.pop("verify_ssl", True)
        )
        session.cert = extra_options.pop("cert", None)
        session.max_redirects = extra_options.pop(
        ...
        try:
            ...
            self.log.warning(
                "Connection to %s has invalid extra field.", connection.host
            )
        return session
...
class HttpAsyncHook(BaseHook):
    ...
    async def run(
        ...
        if self.http_conn_id:
            ...
            if conn.extra:
                extra = _process_extra_options_from_connection(
