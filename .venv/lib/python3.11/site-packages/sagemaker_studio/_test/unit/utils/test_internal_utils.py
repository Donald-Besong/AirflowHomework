...
@patch.object(
    sagemaker_studio.utils,
    "SAGEMAKER_METADATA_JSON_PATH",
...
class TestInternalUtils(unittest.TestCase):
    ...
    def test_get_domain_region_from_environment_variable(self, json_load_mock: Mock):
        json_load_mock.side_effect = json.JSONDecodeError(
            "Error loading file", "Doc", 0
        )
    ...
    @patch("json.load")
    def test_get_domain_region_from_mwaa_space_environment_variable(
        self, json_load_mock: Mock
    ):
        json_load_mock.side_effect = json.JSONDecodeError(
            "Error loading file", "Doc", 0
        )

        os.environ["AIRFLOW__WORKFLOWS__DATAZONE_DOMAIN_REGION"] = (
            "os-environ-mwaa-domain-region"
        )
        region = self.utils._get_domain_region()
    ...
    def test_get_project_id_from_datazone_api(self):
        ...
        self.assertEqual(
            self.utils._get_project_id(
                datazone_api,
                domain_id="bogus_domain_id_00",
                project_name="bogus_project_name_02",
            ),
            "bogus_project_id_02",
        )
    ...
    @patch("json.load")
    def test_get_project_id_from_sagemaker_metadata_json_not_found(
        self, json_load_mock
    ):
        json_load_mock.side_effect = json.JSONDecodeError(
            "Error loading file", "Doc", 0
    ...
    def test_get_user_id_not_found(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Encountered an error getting the current user ID" in context.exception
            )
    ...
    def test_get_project_kms_key_arn_invalid_project_params(self):
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Encountered an error getting the project KMS key" in context.exception
            )
    ...
    def test_get_mlflow_tracking_server_arn_validation_fails(self):
        with patch.object(self.utils, "_get_project_id") as mock:
            ...
            with self.assertRaises(ValueError) as context:
                ...
                self.assertTrue(
                    "One of project_id or project_name must be provided"
                    in context.exception
                )
    ...
    def test_get_mlflow_tracking_server_arn_throws_error(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Encountered an error getting ML flow tracking" in context.exception
            )
    ...
    def test_get_project_s3_path_validation_fails_not_available_in_env(self):
        with patch.object(self.utils, "_get_project_id") as mock_project_id:
            ...
            with patch.object(
                self.utils, "_get_field_from_environment"
            ) as mock_from_env:
                ...
                with self.assertRaises(RuntimeError) as context:
                    ...
                    self.assertTrue(
                        "Encountered an error getting the project S3 path"
                        in context.exception
                    )
    ...
    def test_get_tooling_environment_summary_throws_client_error(self):
        project_api = Mock()
        project_api.get_project_default_environment.return_value = {
            "id": "bogus_env_id"
        }
        datazone_api = Mock()
