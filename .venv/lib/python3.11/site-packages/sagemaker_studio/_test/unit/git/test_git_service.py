...
class TestGitService(unittest.TestCase):
    ...
    def test_get_clone_url_validation(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.git_service.get_clone_url(
                domain_identifier=invalid_domain_id,
                project_identifier=invalid_project_id,
            )
    ...
    def test_get_clone_url_valid(self, mock_get_repository_id, mock_resolve_client):
        ...
        clone_url = self.git_service.get_clone_url(
            "dzd_bx2a2afyvn2hlc", "449et7665a6p3k"
        )
    ...
    def test_resolve_client_with_missing_identifiers(self, mock_get_connection_arn):
        ...
        self.assertEqual(
            str(context.exception), "Domain and project identifiers cannot be None"
        )

    @patch.object(GitService, "_get_connection_arn", return_value=None)
    def test_resolve_client_to_return_git_code_commit_client(
        self, mock_get_connection_arn
    ):
        self.git_service.domain_identifier = "dzd_bx2a2afyvn2hlc"
        self.git_service.project_identifier = "449et7665a6p3k"
        with patch(
            "sagemaker_studio.git.GitCodeCommitClient"
        ) as mock_git_code_commit_client:
    ...
    def test_resolve_client_to_return_git_code_connections_client(
        self, mock_get_connection_arn
    ):
        with patch(
    ...
    @patch.object(GitService, "_get_provisioned_resources")
    def test_get_repository_id_code_repository_name(
        self, mock_get_provisioned_resources
    ):
        mock_get_provisioned_resources.return_value = [
            {"name": "otherResource", "value": "some-value"},
    ...
    @patch.object(GitService, "_get_provisioned_resources")
    def test_get_repository_id_git_full_repository_id(
        self, mock_get_provisioned_resources
    ):
        mock_get_provisioned_resources.return_value = [
    ...
    def test_project_default_environment_none(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.git_service._get_provisioned_resources()
        self.assertEqual(
            str(context.exception), "Project Default Environment Does Not Exist"
        )
    ...
    @patch.object(GitService, "_resolve_client")
    @patch.object(GitService, "_get_repository_id")
    def test_get_clone_url_client_error(
        self, mock_get_repository_id, mock_resolve_client
    ):
        mock_git_client = Mock()
        ...
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Encountered error retrieving clone_url for project"
