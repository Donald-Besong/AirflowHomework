...
class TestConnection(unittest.TestCase):
    ...
    def test_camel_to_snake_case(self):
        ...
        self.assertEqual(
            connection._camel_to_snake("testCamel2Case"), "test_camel2_case"
        )
    ...
    def test_get_connection_catalog_named_catalog_without_account_id_prefix(self):
        ...
        assert (
            connection_catalog.resource_arn
            == GET_CATALOG_RESPONSE["Catalog"]["ResourceArn"]
        )
    ...
    def test_get_connection_catalog_default_catalog(self):
        ...
        connection._glue_api.get_catalog.assert_called_with(
            CatalogId="123456890-Account-Id"
        )
    ...
    def test_get_connection_catalog_default_catalog_no_physical_endpoint_throws_error(
        self,
    ):
        datazone_api_mock = Mock()
        datazone_api_mock.get_connection.return_value = {
        ...
        with self.assertRaises(RuntimeError) as context:
            connection.catalog()
            self.assertTrue(
                "No physical endpoints found for the connection" in context.exception
            )
    ...
    def test_get_connection_catalogs(self):
        ...
        assert any(
            catalog.name == "bogus_catalog_00" for catalog in connection_catalogs
        )
        assert any(
            catalog.name == "bogus_catalog_01" for catalog in connection_catalogs
        )
        assert all(
            catalog.type == "bogus_catalog_type_00" for catalog in connection_catalogs
        )
        assert all(
            "bogus_parent_00_bogus_parent_01_" in catalog.spark_catalog_name
            for catalog in connection_catalogs
        )
        connection._glue_api.get_catalogs.assert_called_with(
            Recursive=True, HasDatabases=True
        )
    ...
    def test_get_connection_catalogs_throws_other_error(self):
        ...
        connection._glue_api = Mock()
        connection._glue_api.get_catalogs.side_effect = AttributeError(
            "Some other error"
        )
    ...
    def test_parse_secret_using_valid_json(self):
        ...
        self.assertEqual(
            connection._parse_secret(secret_1),
            {"username": "user", "password": "password"},
        )

        secret_2 = {"SecretString": '{"username":"user","password": "password"}'}
        self.assertEqual(
            connection._parse_secret(secret_2),
            {"username": "user", "password": "password"},
        )
    ...
    def test_get_secret_client_error_from_secrets_manager(self):
        secret_manager_mock = deepcopy(SECRETS_MANAGER_MOCK)
        connection = Connection(
            {}, GLUE_MOCK, DZ_API_MOCK, SECRETS_MANAGER_MOCK, ClientConfig()
        )
        with patch.object(connection, "_find_secret_arn") as mock:
            mock.return_value = "arn:aws:secretsmanager:us-east-1:secret"
            with patch.object(
                ...
                with self.assertRaises(RuntimeError) as context:
                    connection.secret
                    self.assertTrue(
                        "Encountered an error getting secret for connection"
                        in context.exception
                    )
    ...
    def test_populated_physical_endpoints_with_glue_connection_property(self):
        ...
        self.assertEqual(
            conn_props["HOST"], "https://bogus_host.snowflakecomputing.com"
        )
    ...
    def test_populated_connection_with_data_password(self):
        ...
        self.assertEqual(
            data.credentials.get("usernamePassword").get("password"), "bogus_password"
        )
        data_repr = repr(data)
    ...
    def test_invoke_get_connection_and_populate_fields_successful(self):
        ...
        self.assertEqual(
            connection.physical_endpoints[
                0
            ].glue_connection.authentication_configuration["authenticationType"],
            "BASIC",
        )
    ...
    def test_connection_creds_property(self):
        ...
        self.assertEqual(
            connection.connection_creds.secret_access_key, "secret_access_key"
        )
    ...
    def test_get_secret_successful_for_compute_redshift(self):
        ...
        secret = connection.secret
        self.assertEqual(
            secret, {"username": "bogus_username", "password": "XXXXX_password"}
        )
    ...
    def test_get_aws_client_fails_for_connection_without_default_service(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Please specify a service name to initialize a client"
                in context.exception
            )
