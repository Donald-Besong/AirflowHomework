...
class TestCatalog(unittest.TestCase):
    ...
    def test_get_catalog_database(self):
        ...
        assert (
            catalog_db.location_uri == GET_DATABASE_RESPONSE["Database"]["LocationUri"]
        )
    ...
    def test_get_catalog_databases(self):
        test_catalog = self.get_test_catalog()
        get_databases_paginator = _create_mock_paginator(
            GET_DATABASES_PAGINATED_RESPONSE
        )
        test_catalog._glue_api.get_paginator = Mock()
        test_catalog._glue_api.get_paginator.side_effect = (
            lambda x: get_databases_paginator
        )
    ...
    def test_get_catalog_database_table(self):
        ...
        assert (
            catalog_db_table.database_name
            == GET_TABLE_RESPONSE["Table"]["DatabaseName"]
        )
    ...
    def test_get_catalog_database_tables(self):
        ...
        assert any(
            table.database_name == "bogus_database_00" for table in catalog_db_tables
        )
    ...
    def test_get_catalog_databases_paginator_throws_client_error(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Encountered an error listing databases" in context.exception
            )
    ...
    def test_get_catalog_database_tables_throws_client_error(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            catalog_db.tables
            self.assertTrue(
                "Encountered an error getting tables for database" in context.exception
            )
