...
class TestProject(TestCase):
    def setUp(self):
        ...
        self.list_projects_paginator = _create_mock_paginator(
            LIST_PROJECT_PAGINATED_RESPONSE
        )
        self.mock_datazone_api.get_paginator = Mock()
        self.mock_datazone_api.get_paginator.side_effect = (
            lambda x: self.list_projects_paginator
        )
    ...
    def test_project_initialization_domain_id_not_provided(
        ...
        self.assertEqual(
            project.project_profile_id, GET_PROJECT_RESPONSE["projectProfileId"]
        )
    ...
    def test_project_initialization_only_project_name_provided_project_does_not_exist(
        ...
        with self.assertRaises(RuntimeError) as context:
            ...
            self.assertTrue(
                "Project does_not_exist not found" in str(context.exception)
            )
    ...
    def test_get_project_kms_key_arn_from_default_tooling_env_not_found(
        ...
        with self.assertRaises(RuntimeError) as context:
            project.kms_key_arn
            self.assertTrue(
                "kmsKeyArn provisioned resource not found in Tooling"
                in str(context.exception)
            )
    ...
    @patch("sagemaker_studio.utils._internal.InternalUtils._get_domain_id")
    @patch(
        "sagemaker_studio.utils._internal.InternalUtils._get_mlflow_tracking_server_arn"
    )
    ...
    def get_project_iam_connection_returned_env_usr_role(
        self,
        get_connection_by_name_mock: Mock,
        get_domain_id_mock: Mock,
        get_aws_client_mock: Mock,
    ):
        get_domain_id_mock.return_value = "dzd_1234"
        ...
        get_connection_by_name_mock.return_value = Connection(
            {"environmentUserRole": "arn:aws:iam:env_usr_role"},
            Mock(),
            Mock(),
            Mock(),
            Mock(),
    ...
    def test_get_user_id_from_sagemaker_json_file_not_exists(
        ...
        with self.assertRaises(RuntimeError) as context:
            project.user_id
            self.assertTrue(
                "Encountered an error getting the current user ID"
                in str(context.exception)
            )
