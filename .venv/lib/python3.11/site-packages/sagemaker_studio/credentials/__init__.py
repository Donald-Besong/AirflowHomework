...
class CredentialsVendingService:
    ...
    def get_domain_execution_role_credential_in_space(self, domain_identifier: str):
        ...
        return {
            "Version": 1,
            "AccessKeyId": domain_exec_creds.get("credentials", {}).get(
                "accessKeyId", ""
            ),
            "SecretAccessKey": domain_exec_creds.get("credentials", {}).get(
                "secretAccessKey", ""
            ),
            "SessionToken": domain_exec_creds.get("credentials", {}).get(
                "sessionToken"
            ),
