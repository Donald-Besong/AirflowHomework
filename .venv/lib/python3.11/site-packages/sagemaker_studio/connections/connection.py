...
class Connection:
    ...
    def __init__(
        ...
        connection_credentials = self.__connection_data.get(
            "connectionCredentials", None
        )
        self._connection_creds = self._map_connection_credentials_field(
        ...
        self.physical_endpoints = [
            ...
            for endpoint in physical_endpoints_data
        ]
        ...
        if self.type in ["LAKEHOUSE", "IAM"]:
            self._glue_api: BaseClient = (
                self._get_aws_client_with_connection_credentials(
    ...
    def catalogs(self) -> List[Catalog]:
        ...
        if self.type not in ["LAKEHOUSE", "IAM"]:
            raise AttributeError(
                "Catalogs are only available for IAM or LAKEHOUSE connections"
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Encountered an error getting catalogs for the connection",
    ...
    def catalog(self, id: Optional[str] = None) -> Catalog:
        ...
        try:
            ...
            if not self._validate_catalog_id(id):
                id = (
                    f"{aws_account_id}:{id}"
                    if not self._validate_aws_account_id(id)
                    else id
                )
            ...
            raise RuntimeError(
                f"Encountered an error getting catalog '{id or aws_account_id}", e
            )
    ...
    def secret(self) -> Union[Dict[str, str], str]:
        ...
        if len(self.physical_endpoints) >= 1 and self.physical_endpoints[0].glue_connection:  # type: ignore
            ...
            if hasattr(self.data, "credentials") and self.data.credentials.get(
                "usernamePassword"
            ):
    ...
    def _find_secret_arn(self) -> str:
        ...
        if len(self.physical_endpoints) >= 1 and self.physical_endpoints[0].glue_connection:  # type: ignore
            ...
            auth_config = getattr(
                endpoint.glue_connection, "authentication_configuration", None
            )
            if auth_config and isinstance(auth_config, dict):
                secret_arn = auth_config.get("secretArn")
        if not secret_arn:
            raise AttributeError(
                f"Connection {self.name} does not have associated secret"
            )
        return secret_arn
    ...
    def _map_connection_credentials_field(
        ...
        if not creds:
            return None
        snake_case_creds = {
            self._camel_to_snake(key): value for key, value in creds.items()
        }
        if "expiration" in snake_case_creds and isinstance(
            snake_case_creds["expiration"], str
        ):
            snake_case_creds["expiration"] = datetime.fromisoformat(
                snake_case_creds["expiration"]
            )
        return ConnectionCredentials(**snake_case_creds)

    def _map_physical_endpoint_fields(
        ...
        snake_case_endpoint["iam_connection_id"] = aws_location.get(
            "iamConnectionId", None
        )
        ...
        if glue_connection_data:
            glue_connection_class = make_dataclass(
                "GlueConnection",
                [
                    (self._camel_to_snake(k), type(v), None)
                    for k, v in glue_connection_data.items()
                ],
            )
            ...
            snake_case_endpoint["host"] = glue_connection_data.get(
                "connectionProperties", {}
            ).get("HOST", None)
            snake_case_endpoint["port"] = glue_connection_data.get(
                "connectionProperties", {}
            ).get("PORT", None)
    ...
    def _create_connection_data(self, connection_data: Dict[str, Any]):
            ...
            if key == "props" and isinstance(value, dict):
                for sub_key, sub_value in value.items():
                    ...
                    if isinstance(sub_value, dict):
                        ...
                        connection_data_snake_case[self._camel_to_snake(sub_key)] = (
                            sub_value
                        )
                ...
                if isinstance(value, (dict, list)):
                    ...
                    fields.append(
                        (self._camel_to_snake(key), type(value), field(default=value))
                    )
        ...
        def custom_repr_for_credentials_password(self):
            ...
            if (
                data_dict.get("credentials")
                and "usernamePassword" in data_dict["credentials"]
            ):
                data_dict["credentials"]["usernamePassword"]["password"] = "****"
            data_fields = []
            for data_key, data_value in data_dict.items():
                if (
                    isinstance(data_value, list) or isinstance(data_value, dict)
    ...
    def _get_aws_client_with_connection_credentials(
        ...
        if connection_credentials is not None:
            if any(
                ...
                raise RuntimeError(
                    "Unable to find credentials for project.iam connection"
                )
            region_name = existing_client.meta.region_name
            project_override_config = self._project_config.overrides.get(
                service_name, {}
            )
        ...
        raise RuntimeError(
            f"Encountered an error getting AWS client for {service_name}."
        )

    def _catalog_builder(self, get_catalog_response) -> Catalog:
        catalog_name = get_catalog_response.get("Name")
        parent_catalog_names = "_".join(
            get_catalog_response.get("ParentCatalogNames", [])
        )
        spark_catalog_name = (
            f"{parent_catalog_names}_{catalog_name}"
            if parent_catalog_names
            else catalog_name
        )
        return Catalog(
            name=catalog_name,
    ...
    def _invoke_get_connection_and_populate_fields(self):
        try:
            ...
            connection_credentials = connection_response.get(
                "connectionCredentials", None
            )
            self._connection_creds = self._map_connection_credentials_field(
                connection_credentials
