...
class InternalUtils:
    ...
    def _get_project_id(
        ...
        if project_name and domain_id:
            ...
            try:
                project_page_iterator = project_paginator.paginate(
    ...
    def _get_project_s3_path(
        ...
        try:
            ...
            self.__validate_domain_project_parameters(
                domain_id, project_id, project_name
            )
            ...
            default_tooling_environment_provisioned_resources = (
                default_tooling_environment_environment_summary.get(
    ...
    def _get_mlflow_tracking_server_arn(
        ...
        try:
            ...
            for page in environment_page_iterator:
                for environment in page.get("items", []):
                    if environment.get("name") == "MLExperiments":
                        sagemaker_studio_ml_experiments_environment_id = (
                            environment.get("id")
                        )
            ...
            raise RuntimeError(
                "Encountered an error getting ML flow tracking server ARN", e
            )
    ...
    def _get_field_from_sagemaker_space_metadata_json(self, field: str) -> str:
        try:
            with open(
                ...
                return sagemaker_space_metadata.get(
                    field
                ) or sagemaker_space_metadata.get("AdditionalMetadata", {}).get(field)
            ...
            raise RuntimeError(
                "An unexpected error occurred when accessing SageMaker resource metadata",
                e,
            )
    ...
    def _get_default_tooling_environment_summary(
        ...
        try:
            default_tooling_environment_id = (
                project_api.get_project_default_environment(
