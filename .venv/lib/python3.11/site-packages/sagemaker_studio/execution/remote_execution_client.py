...
DEFAULT_INSTANCE_TYPE = (
    "ml.m6i.xlarge"  # consistent with the default instance in the toolkit
)
...
class RemoteExecutionClient(ExecutionClient):
    ...
    def _get_execution_internal(self, request: GetExecutionRequest) -> dict:
        ...
        try:
            describe_training_job_response = (
                self.sagemaker_client.describe_training_job(
            ...
            if (
                ...
                raise RuntimeError(
                    f"Error getting execution with id: {request.execution_id}"
                )
            ...
            return {
                key: value
                for (key, value) in get_execution_response.items()
            ...
            if error_code == "ValidationException":
                if (
                    "Requested resource not found." in error_message
                    ...
                    raise InternalServerError(
                        f"Invalid input parameters: {AWSClientException(e)}"
                    )
            else:
    ...
    def _list_executions_internal(self, request: ListExecutionsRequest) -> dict:
        ...
        try:
            ...
            if request.get("sort_order"):
                search_sort_order = ExecutionUtils.map_sort_order(
                    SortOrder(request.sort_order)
                )
                search_args["SortOrder"] = search_sort_order
            search_expression = (
                ExecutionUtils.create_sagemaker_search_expression_for_training(
        ...
        if "Results" in search_response:
            ...
            list_executions_response["next_token"] = search_response.get(
                "NextToken", None
            )
    ...
    def _start_execution_internal(self, request: StartExecutionRequest):
        ...
        if request.get("tags"):
            execution_tags.extend(
                {"Key": key, "Value": value}
                for key, value in request.tags.to_dict().items()
            )
        ...
        if "ecr_uri" in image_details:
            ...
            image_version: str = image_details.get(
            ...
            if image_name != image_name_default:
                ...
                if account_id == "":
                    raise InternalServerError(
                        "Account ID not found in SSM parameter store"
                    )
        ...
        if "output_config" in request and "notebook_config" in request["output_config"]:
            request["output_config"]["notebook_config"][
                "output_formats"
            ] = output_formats
        else:
            request["output_config"] = {
                "notebook_config": {"output_formats": output_formats}
            }
        ...
        try:
            ...
            create_training_job_response = self.sagemaker_client.create_training_job(
                ...
                Environment={
                    "AWS_DEFAULT_REGION": self.default_tooling_environment[
                        "awsAccountRegion"
                    ],
                    ...
                    "SM_OUTPUT_FORMATS": (
                        ",".join(output_formats_lowercase)
                        if output_formats_lowercase
                        else ""
                    ),
            ...
            split_arn = create_training_job_response.get("TrainingJobArn").split(
                ":training-job/"
            )
            if len(split_arn) != 2:
                raise RuntimeError("Remote executionId not available")
            split_arn = create_training_job_response["TrainingJobArn"].split(
                ":training-job/"
            )
            ...
            raise RuntimeError(
                f"Error starting remote execution: {AWSClientException(e)}"
            )
    ...
    def __set_default_tooling_environment(self):
        self.default_tooling_environment = (
            self.project_api.get_project_default_environment(
    ...
    def __setup_execution_client(self):
        ...
        if not self.config.domain_identifier or not self.config.project_identifier:
            raise InternalServerError(
                "Domain identifier and project identifier are required"
            )
    ...
    def validate_input_config(input_config: dict):
        ...
        if "notebook_config" not in input_config:
            raise ValidationError(
                "notebookConfig in InputConfig is required for remote execution"
            )
        ...
        validate_input_parameters(
            input_config["notebook_config"].get("input_parameters")
        )
