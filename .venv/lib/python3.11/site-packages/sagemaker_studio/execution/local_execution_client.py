...
class LocalExecutionClient(ExecutionClient):
    ...
    def _get_execution_internal(self, request: GetExecutionRequest) -> dict:
        ...
        try:
            ...
            if (
                job_data["status"]
                in LocalExecutionTerminalStatusesWithOutputFiles.__members__
            ):
                response = {
                    "execution_id": job_id,
            ...
            raise LocalExecutionUtils.get_api_error(
                e, "GetExecution", request.execution_id
            )
    ...
    def _list_executions_internal(self, request: ListExecutionsRequest) -> dict:
        try:
            search_sort_order = (
                ...
                and request.get("sort_order") == "ASCENDING"
                else "desc"
            )
            jupyter_scheduler_sort_by = (
                LocalExecutionUtils.get_jupyter_scheduler_sort_by(
                    sort_order=search_sort_order, sort_by=request.get("sort_by")
                )
            )
            query_params = {
                "start_time": (
                    request.start_time_after
                    if request.get("start_time_after")
                    else None
                ),
                "name": request.name_contains if request.get("name_contains") else None,
                "status": (
                    Status(request.status).value if request.get("status") else None
                ),
                "sort_by": (
                    jupyter_scheduler_sort_by if jupyter_scheduler_sort_by else None
                ),
                "next_token": request.next_token if request.get("next_token") else None,
                "max_items": (
                    request.max_results if request.get("max_results") else None
                ),
            }
            query_params = {
    ...
    def _start_execution_internal(self, request: StartExecutionRequest) -> dict:
        ...
        bytes_free = LocalExecutionUtils.get_bytes_free(
        ...
        output_formats_lowercase = [
            output_format.lower().replace("notebook", "ipynb")
        ...
        try:
            ...
            if "output_formats" not in request.get("output_config", {}).get(
                "notebook_config", {}
            ):
                response.setdefault("output_config", {}).setdefault(
                    "notebook_config", {}
    ...
    def _stop_execution_internal(self, request: StopExecutionRequest) -> None:
        ...
        try:
            ...
            raise LocalExecutionUtils.get_api_error(
                e, "StopExecution", request.execution_id
            )
