...
class ExecutionUtils:
    ...
    def map_training_job_status_to_status(training_job_status: str):
        if training_job_status == "InProgress":
            ...
            raise RuntimeError(
                f"Invalid training job status defined: {training_job_status}"
            )
    ...
    def create_sagemaker_search_expression_for_training(
        ...
        filters: List[Dict[str, Any]] = [
            {
                "Name": "Tags.sagemaker-notebook-execution",
                "Operator": "Equals",
                "Value": "TRUE",
            },
            {
            ...
            {
                "Name": "Tags.AmazonDataZoneDomain",
                "Operator": "Equals",
...
class LocalExecutionUtils:
    ...
    LOCAL_JOBS_SCHEDULER_URL = (
        f"{LOCAL_EXECUTION_HOST}/jupyterlab/default/scheduler/jobs"
    )
    ...
    def download_job_files(jobs_url: str, job_id: str, redownload: bool = False):
        ...
        try:
            ...
            raise RuntimeError(
                f"Exception occurred when downloading output files of {job_id}: {e}"
            )
    ...
    @staticmethod
    def get_api_error(
        exception: Exception, api: str, reference_id: Optional[str] = None
    ):
        ...
        if isinstance(exception, RequestError):
            ...
            if http_status_code == 404:
                ...
                return ConflictError(
                    f"{general_message}: Execution with token already exists"
                )
            else:
            ...
            return InternalServerError(
                f"{general_message}: An unknown error occurred: {exception}"
            )
...
class RemoteExecutionUtils:
    ...
    def pack_s3_path_for_input_file(
        ...
        if is_git_project:
            ...
            project_s3_path = (
                project_s3_path[:-3] + "shared"
            )  # replace 'dev' with 'shared'
