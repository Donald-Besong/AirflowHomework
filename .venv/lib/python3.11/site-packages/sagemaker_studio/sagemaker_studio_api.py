...
class SageMakerStudioAPI:
    ...
    def __init__(self, config: ClientConfig = ClientConfig()):
        ...
        self.default_region: str = (
            self.sagemaker_studio_config.region
            or self._utils._get_domain_region()
            or "us-east-1"
        )
        ...
        if self.sagemaker_studio_config.profile_name:
        ...
        elif os.path.exists(space_aws_config_path) and os.path.exists(
            ...
            with open(space_aws_config_path, "r") as space_aws_config_config_file:
                ...
                profile_name = (
                    ...
                    if domain_execution_role_creds_profile_name
                    in space_aws_config_config
                    else None
                )
        else:
            profile_name = None
        ...
        self.secrets_manager_api = self._get_aws_client(
            "secretsmanager",
            self.sagemaker_studio_config.overrides.get("secretsmanager", {}),
        )
        self.project_api = ProjectService(self.datazone_api)
        self.credentials_api = CredentialsVendingService(
            self.datazone_api, self.project_api
        )
    ...
    def _build_execution_config(self):
        ...
        if execution_config.domain_identifier:
            execution_config.project_identifier = self._utils._get_project_id(
                self.datazone_api
            )
        execution_config.datazone_stage = self._utils._get_datazone_stage()
        execution_config.datazone_endpoint = self._utils._get_datazone_endpoint(
            self.default_region
        )
        execution_config.datazone_environment_id = (
            self._utils._get_datazone_environment_id()
        )
        ...
        if "environment" in service_override_config:
            ...
            get_environment_credentials_response: dict = (
                self.datazone_api.get_environment_credentials(
                    domainIdentifier=service_override_config["environment"][
                        "domain_identifier"
                    ],
            ...
            secret_access_key = get_environment_credentials_response[
    ...
    def _set_boto3_models_path_env_var(self):
        ...
        if "sagemaker_studio.zip" in models_path:
            try:
                ...
                models_path = str(
                    os.path.join(zip_parent_path, "sagemaker_studio/boto3_models")
                )
            except Exception:
