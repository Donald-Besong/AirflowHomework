...
class Test_Csv(unittest.TestCase):
    ...
    def _test_arg_valid(self, ctor, arg):
        ...
        self.assertRaises(py_compat_exc, ctor, arg, quoting=csv.QUOTE_ALL, quotechar="")
        self.assertRaises(
            py_compat_exc, ctor, arg, quoting=csv.QUOTE_ALL, quotechar=None
        )
    ...
    def _test_default_attrs(self, ctor, *args):
        ...
        self.assertRaises(py_compat_exc, setattr, obj.dialect, "quoting", None)
    ...
    def _write_test(self, fields, expect, **kwargs):
        ...
        try:
            ...
            fileobj.seek(0)
            self.assertEqual(
                fileobj.read(), expect + writer.dialect.lineterminator.encode("utf-8")
            )
        finally:
            fileobj.close()
            os.unlink(name)
    ...
    def test_write_arg_valid(self):
        ...
        self.assertRaises(
            csv.Error, self._write_test, [None], None, quoting=csv.QUOTE_NONE
        )
    ...
    def test_write_bigfield(self):
        ...
        bigstring = "X" * 50000
        self._write_test(
            [bigstring, bigstring], b",".join([bigstring.encode("utf-8")] * 2)
        )
    ...
    def test_write_quoting(self):
        ...
        self._write_test(["a", 1, "p,q"], b'a,1,"p,q"', quoting=csv.QUOTE_MINIMAL)
        self._write_test(["a", 1, "p,q"], b'"a",1,"p,q"', quoting=csv.QUOTE_NONNUMERIC)
        self._write_test(["a", 1, "p,q"], b'"a","1","p,q"', quoting=csv.QUOTE_ALL)
        self._write_test(["a\nb", 1], b'"a\nb","1"', quoting=csv.QUOTE_ALL)

    def test_write_decimal(self):
        self._write_test(
            ["a", decimal.Decimal("1.1"), "p,q"],
            b'"a",1.1,"p,q"',
            quoting=csv.QUOTE_NONNUMERIC,
        )

    def test_write_escape(self):
        self._write_test(["a", 1, "p,q"], b'a,1,"p,q"', escapechar="\\")
        self.assertRaises(
            csv.Error,
            self._write_test,
            ["a", 1, 'p,"q"'],
        ...
        self._write_test(
            ["a", 1, 'p,"q"'], b'a,1,"p,\\"q\\""', escapechar="\\", doublequote=False
        )
        self._write_test(['"'], b'""""', escapechar="\\", quoting=csv.QUOTE_MINIMAL)
        self._write_test(
            ['"'], b'\\"', escapechar="\\", quoting=csv.QUOTE_MINIMAL, doublequote=False
        )
        self._write_test(['"'], b'\\"', escapechar="\\", quoting=csv.QUOTE_NONE)
        self._write_test(
            ["a", 1, "p,q"], b"a,1,p\\,q", escapechar="\\", quoting=csv.QUOTE_NONE
        )
    ...
    def test_read_oddinputs(self):
        self._read_test([], [])
        self._read_test([b""], [[]])
        self.assertRaises(csv.Error, self._read_test, [b'"ab"c'], None, strict=1)
        # cannot handle null bytes for the moment
        self.assertRaises(csv.Error, self._read_test, [b"ab\0c"], None, strict=1)
    ...
    def test_read_quoting(self):
        ...
        self._read_test(
            [b'1,",3,",5'], [["1", '"', "3", '"', "5"]], quotechar=None, escapechar="\\"
        )
        self._read_test(
            [b'1,",3,",5'],
            [["1", '"', "3", '"', "5"]],
        ...
        self._read_test(
            [b',3,"5",7.3, 9'], [["", 3, "5", 7.3, 9]], quoting=csv.QUOTE_NONNUMERIC
        )
        self._read_test([b'"a\nb", 7'], [["a\nb", " 7"]])
        self.assertRaises(
            ValueError, self._read_test, [b"abc,3"], [[]], quoting=csv.QUOTE_NONNUMERIC
        )

    def test_read_linenum(self):
        for r in (
            csv.reader([b"line,1", b"line,2", b"line,3"]),
            csv.DictReader(
                [b"line,1", b"line,2", b"line,3"], fieldnames=["a", "b", "c"]
            ),
        ):
...
class TestDialectRegistry(unittest.TestCase):
    ...
    def test_register_kwargs(self):
        ...
        try:
            ...
            self.assertEqual(list(csv.reader([b"X;Y;Z"], name)), [["X", "Y", "Z"]])
        finally:
            csv.unregister_dialect(name)
...
class TestDialectExcel(TestCsvBase):
    ...
    def test_quoted(self):
        self.readerAssertEqual(
            b'1,2,3,"I think, therefore I am",5,6',
...
9,8,7,6'''
    ...
    def test_simple_writer(self):
        self.writerAssertEqual([[1, 2, "abc", 3, 4]], b"1,2,abc,3,4\r\n")

    def test_quotes(self):
        self.writerAssertEqual([[1, 2, 'a"bc"', 3, 4]], b'1,2,"a""bc""",3,4\r\n')
...
class TestDictFields(unittest.TestCase):
    ...
    def test_write_unicode_header_dict(self):
        ...
        fileobj = open(name, "w+b")
        try:
            writer = csv.DictWriter(fileobj, fieldnames=["ñ", "ö"])
            writer.writeheader()
            fileobj.seek(0)
            self.assertEqual(fileobj.readline().decode("utf-8"), "ñ,ö\r\n")
        finally:
            fileobj.close()
            os.unlink(name)
...
class TestUnicodeErrors(unittest.TestCase):
    ...
    def test_decode_error(self):
        ...
        file = EncodedFile(
            BytesIO("Löwis,2,3".encode("iso-8859-1")), data_encoding="iso-8859-1"
        )
