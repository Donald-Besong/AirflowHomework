...
class SparkCursor(SparkBaseCursor, WithCalculationExecution):
    ...
    def get_std_out(self) -> Optional[str]:
        if (
            not self._calculation_execution
            or not self._calculation_execution.std_out_s3_uri
        ):
            return None
    ...
    def get_std_error(self) -> Optional[str]:
        if (
            not self._calculation_execution
            or not self._calculation_execution.std_error_s3_uri
        ):
            return None
    ...
    def execute(
        ...
        **kwargs,
        ...
        if (
            self._calculation_execution.state
