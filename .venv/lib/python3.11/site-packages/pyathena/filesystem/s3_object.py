        ...
        **kwargs,
        ...
        if init:
            ...
            if "Size" in init:
                ...
                self.size = 0
        super().update(
            {_API_FIELD_TO_S3_OBJECT_PROPERTY.get(k, k): v for k, v in kwargs.items()}
        )
...
class S3MultipartUploadPart:
    def __init__(self, part_number: int, response: Dict[str, Any]) -> None:
        ...
        self._server_side_encryption: Optional[str] = response.get(
            "ServerSideEncryption"
        )
        self._sse_customer_algorithm: Optional[str] = response.get(
