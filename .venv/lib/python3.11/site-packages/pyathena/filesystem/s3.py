...
class S3FileSystem(AbstractFileSystem):
    ...
    def _get_client_compatible_with_s3fs(self, **kwargs) -> BaseClient:
        ...
        return session.client(
            ...
            **{
                ...
                if k in Connection._CLIENT_PASSING_ARGS
            },
        )
    ...
    def info(self, path: str, **kwargs) -> S3Object:
        ...
        bucket, key, path_version_id = self.parse_path(path)
        version_id = (
            path_version_id if path_version_id else kwargs.pop("version_id", None)
        )
        if path in ["/", ""]:
            return S3Object(
                init={
        ...
        if key:
            object_info = self._head_object(
                path, refresh=refresh, version_id=version_id
            )
            if object_info:
                return object_info
        else:
    ...
    def _delete_objects(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for delete in [
                ...
                fs.append(
                    executor.submit(
                        self._call, self._client.delete_objects, **request, **kwargs
                    )
                )
    ...
    def touch(self, path: str, truncate: bool = True, **kwargs) -> Dict[str, Any]:
        ...
        if not truncate and self.exists(path):
            raise ValueError(
                "Cannot touch the existing file without specifying truncate."
            )
        if not key:
    ...
    def sign(self, path: str, expiration: int = 3600, **kwargs):
        ...
        _logger.debug(
            f"Generate signed url: s3://{bucket}/{key}?versionId={version_id}"
        )
        return self._call(
    ...
    def _get_object(
        ...
        **kwargs,
        ...
        _logger.debug(
            f"Get object: s3://{bucket}/{key}?versionId={version_id}&range={range_}"
        )
        response = self._call(
    ...
    def _put_object(
        self, bucket: str, key: str, body: Optional[bytes], **kwargs
    ) -> S3PutObject:
    ...
    def _upload_part(
        ...
        **kwargs,
        ...
        _logger.debug(
            f"Upload part of {upload_id} to s3://{bucket}/{key} as part {part_number}."
        )
        response = self._call(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "r" not in mode and block_size < self.fs.MULTIPART_UPLOAD_MIN_PART_SIZE:
            ...
            raise ValueError(
                f"Block size must be >= {self.fs.MULTIPART_UPLOAD_MIN_PART_SIZE}MB."
            )
            ...
            if self.tell() > S3FileSystem.MULTIPART_UPLOAD_MAX_PART_SIZE:
                    ...
                    self.multipart_upload_parts.append(
                        self._executor.submit(
                            ...
                            upload_id=cast(
                                str,
                                cast(
                                    S3MultipartUpload, self.multipart_upload
                                ).upload_id,
                            ),
                            part_number=i + 1,
                            copy_source_ranges=range_,
                ...
                self.multipart_upload_parts.append(
                    self._executor.submit(
                        ...
                        upload_id=cast(
                            str,
                            cast(S3MultipartUpload, self.multipart_upload).upload_id,
                        ),
                        part_number=1,
                    )
                )
