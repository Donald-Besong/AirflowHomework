...
class AthenaStatementCompiler(SQLCompiler):
    ...
    def visit_cast(self, cast: "Cast[Any]", **kwargs):
        if (
            isinstance(cast.type, types.VARCHAR) and cast.type.length is None
        ) or isinstance(cast.type, types.String):
            type_clause = "VARCHAR"
...
class AthenaDDLCompiler(DDLCompiler):
    ...
    def visit_check_constraint(
        self, constraint: "CheckConstraint", **kw
    ) -> Optional[str]:
        return None
    ...
    def visit_unique_constraint(
        self, constraint: "UniqueConstraint", **kw
    ) -> Optional[str]:
        return None
...
class AthenaDialect(DefaultDialect):
    ...
    _pattern_column_type: Pattern[str] = re.compile(
        r"^([a-zA-Z]+)(?:$|[\(|<](.+)[\)|>]$)"
    )
    ...
    def _create_connect_args(self, url: "URL") -> Dict[str, Any]:
        ...
        if "kill_on_interrupt" in opts:
            opts.update(
                {"kill_on_interrupt": bool(strtobool(opts["kill_on_interrupt"]))}
            )
        if "result_reuse_enable" in opts:
            opts.update(
                {"result_reuse_enable": bool(strtobool(opts["result_reuse_enable"]))}
            )
    ...
    def get_table_names(
        ...
        return [
            ...
            for t in tables
            if t.table_type
            in ["EXTERNAL_TABLE", "MANAGED_TABLE", "EXTERNAL", "customer"]
        ]
