...
class BaseCursor(metaclass=ABCMeta):
    ...
    def _build_list_query_executions_request(
        ...
        request: Dict[str, Any] = {
            "MaxResults": (
                max_results if max_results else self.LIST_QUERY_EXECUTIONS_MAX_RESULTS
            )
        }
        if self._work_group or work_group:
            request.update(
    ...
    def _build_list_table_metadata_request(
        ...
        request: Dict[str, Any] = {
            ...
            "MaxResults": (
                max_results if max_results else self.LIST_TABLE_METADATA_MAX_RESULTS
            ),
        }
        if expression:
    ...
    def _build_list_databases_request(
        ...
        request: Dict[str, Any] = {
            "CatalogName": catalog_name if catalog_name else self._catalog_name,
            "MaxResults": (
                max_results if max_results else self.LIST_DATABASES_MAX_RESULTS
            ),
        }
        if next_token:
            request.update({"NextToken": next_token})
    ...
    def _list_databases(
        ...
        try:
            ...
            return response.get("NextToken"), [
                AthenaDatabase({"Database": r})
                for r in response.get("DatabaseList", [])
            ]
    ...
    def _get_calculation_execution_status(
        self, query_id: str
    ) -> AthenaCalculationExecutionStatus:
    ...
    def _execute(
        ...
        if pyathena.paramstyle == "qmark" or paramstyle == "qmark":
            ...
            query = self._formatter.format(
                operation, cast(Optional[Dict[str, Any]], parameters)
            )
