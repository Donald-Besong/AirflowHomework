...
class CloudWatchLogHandler(logging.Handler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if log_group is not None:
            if log_group_name != __name__:
                raise WatchtowerError(
                    "Both log_group_name and deprecated log_group parameter specified"
                )
            warnings.warn(
                "Please use log_group_name instead of log_group", DeprecationWarning
            )
            self.log_group_name = log_group
        if stream_name is not None:
            if log_stream_name != DEFAULT_LOG_STREAM_NAME:
                raise WatchtowerError(
                    "Both log_stream_name and deprecated stream_name parameter specified"
                )
            warnings.warn(
                "Please use log_stream_name instead of stream_name", DeprecationWarning
            )
            self.log_stream_name = stream_name

        self.setFormatter(
            CloudWatchLogFormatter(json_serialize_default=json_serialize_default)
        )
        ...
        if boto3_client is None and boto3_profile_name is None:
            ...
            self.cwl_client = boto3.session.Session(
                profile_name=boto3_profile_name
            ).client("logs")
        else:
            raise WatchtowerError(
                "Either boto3_client or boto3_profile_name can be specified, but not both"
            )
    ...
    def _ensure_log_group(self):
        try:
            ...
            for log_group in self._paginate(
                paginator, logGroupNamePrefix=self.log_group_name
            ):
    ...
    def _tag_log_group(self, log_group_tags: Dict[str, str]):
        try:
            self._idempotent_call(
                "tag_resource",
                resourceArn=self._get_log_group_arn(),
                tags=log_group_tags,
            )
        except (
            ...
            warnings.warn(
                f"Failed to tag log group {self.log_group_name}: {e}", WatchtowerWarning
            )
    ...
    def _truncate(self, msg, max_size):
        ...
        msg["message"] = (
            msg["message"].encode("utf-8")[:max_size].decode("utf-8", "ignore")
        )
        return msg
        ...
        for retry in range(max_retries):
            try:
                ...
                if isinstance(
                    ...
                    next_expected_token = e.response["Error"]["Message"].rsplit(" ", 1)[
                        -1
                    ]
                    ...
                    e, self.cwl_client.exceptions.ResourceNotFoundException
                ):
    ...
    def emit(self, message):
        ...
        try:
            ...
            cwl_message = dict(
                timestamp=int(message.created * 1000), message=self.format(message)
            )

            max_message_body_size = (
                self.max_message_size - CloudWatchLogHandler.EXTRA_MSG_PAYLOAD_SIZE
            )
    ...
    def _dequeue_batch(
        self, my_queue, stream_name, send_interval, max_batch_size, max_batch_count
    ):
        msg = None
        ...
        while msg != self.END:
            ...
            while True:
                try:
                    msg = my_queue.get(
                        block=True, timeout=max(0, cur_batch_deadline - time.time())
                    )
                except queue.Empty:
        ...
        for q in self.queues.values():
            with q.all_tasks_done:
                q.all_tasks_done.wait_for(
                    lambda: q.unfinished_tasks == 0, timeout=self.FLUSH_TIMEOUT
                )
